...
void BBoxDataHandKeypointLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  int num_limbs=20;
  int num_parts=21;
  top[1]->Reshape(1,1,1,9+batch_size*num_parts*channelOffset);
  for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
    this->prefetch_[i].label_.Reshape(1,1,1,9+num_parts*channelOffset);
  }
...
void BBoxDataHandKeypointLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  // Dtype* top_labels = batch->label_.mutable_cpu_data();
    ...
    // batch->label_.Reshape(1,1,1,9+batch_size*(4*num_limbs+2*num_parts)*channelOffset);
      Blob<Dtype> blob_tmp;
    blob_tmp.Reshape(1,1,1,batch_size*num_parts*channelOffset);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int old=num_parts*item_id;
    Dtype* transformed_heatmap = blob_tmp.mutable_cpu_data()+old*channelOffset;
   
    bbox_data_transformer_->Transform(anno, &image, &bboxes, &crop_bbox, &doflip,transformed_heatmap);
    num_p += bboxes.size();
  ...
  batch->label_.Reshape(1,1,1,num_gt*9+batch_size*num_parts*channelOffset);
    Dtype* top_pose = batch->label_.mutable_cpu_data();
caffe_copy<Dtype>(batch_size*num_parts*channelOffset, blob_tmp.mutable_cpu_data(),top_pose);
...
//else LOG(INFO)<<t;
 ...
 // caffe_copy<Dtype>(batch_size*(4*num_limbs+2*num_parts)*channelOffset, blob_tmp.mutable_cpu_data(),top_pose);
  Dtype* top_label = batch->label_.mutable_cpu_data()+batch_size*num_parts*channelOffset;
  int count = 0;
  int idx = 0;
  for (int i = 0; i < bboxes_all.size(); ++i) {
...
void BBoxDataHandKeypointLayer<Dtype>::ReadAnnoDataFromXml(const int bindex, const string& xml_file, const string& root_dir,
  ...
  for (int i = 0; i < anno->num_person; ++i) {
    ...
    try {
      ...
      ins.joint.joints.resize(21);
      ins.joint.isVisible.resize(21);
      for(int j = 0; j < 21; ++j) {
