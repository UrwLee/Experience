#ifndef CAFFE_MASK_BBOX_DATA_LAYER_PUTNOCROP_HPP_
#define CAFFE_MASK_BBOX_DATA_LAYER_PUTNOCROP_HPP_
...
class BBoxDataPutNoCropLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit BBoxDataPutNoCropLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param), bbox_data_transform_param_(param.unified_data_transform_param()) {}
  virtual ~BBoxDataPutNoCropLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "BBoxDataPutNoCrop"; }
 ...
 protected:
  ...
  // 数据转换器
  shared_ptr<BBoxDataTransformerPutNoCropLayer<Dtype> > bbox_data_transformer_;
