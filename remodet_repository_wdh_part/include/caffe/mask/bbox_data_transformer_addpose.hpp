#ifndef CAFFE_MASK_BBOX_DATA_TRANSFORMER_ADDPOSE_HPP_
#define CAFFE_MASK_BBOX_DATA_TRANSFORMER_ADDPOSE_HPP_
...
class BBoxDataAddPoseTransformer {
 public:
  // 构造函数
  // 我们仍然使用UnifiedTransformationParameter进行构造
  explicit BBoxDataAddPoseTransformer(const UnifiedTransformationParameter& param, Phase phase)
      : param_(param), phase_(phase) { InitRand(); }
  virtual ~BBoxDataAddPoseTransformer() {}
  ...
  void Transform(AnnoData<Dtype>& anno, cv::Mat* image, bool* doflip,cv::Mat* maskmiss);
  void randomExpand(AnnoData<Dtype>& anno, cv::Mat* image,cv::Mat* maskmiss);

  void randomCrop(AnnoData<Dtype>& anno, cv::Mat* image, BoundingBox<Dtype>* crop_bbox,cv::Mat* maskmiss);
  void randomCrop(AnnoData<Dtype>& anno, cv::Mat* image,cv::Mat* maskmiss);

  void getCropBBox(AnnoData<Dtype>& anno, BoundingBox<Dtype>* crop_bbox);

  void TransCrop(AnnoData<Dtype>& anno,const BoundingBox<Dtype>& crop_bbox,cv::Mat* image,cv::Mat* maskmiss);

  void randomDistortion(cv::Mat* image, AnnoData<Dtype>& anno);

  void randomFlip(AnnoData<Dtype>& anno, cv::Mat& image, bool* doflip,cv::Mat& maskmiss);
  ...
  void fixedResize(AnnoData<Dtype>& anno, cv::Mat& image,cv::Mat& maskmiss);
  ...
  void test(AnnoData<Dtype>& anno, cv::Mat& image,cv::Mat& maskmiss);
  void get_mask(AnnoData<Dtype>& anno, cv::Mat& image,cv::Mat& maskmiss,Dtype* transformed_vec_mask,Dtype* transformed_heat_mask,Dtype* transformed_vecmap,Dtype* transformed_heatmap);
  void generateLabelMap(Dtype* transformed_vecmap, Dtype* transformed_heatmap, cv::Mat& img_aug, AnnoData<Dtype>& anno);
