#ifndef CAFFE_MINIHAND_SAMPLE_TRANSFORMER_HPP_
#define CAFFE_MINIHAND_SAMPLE_TRANSFORMER_HPP_
...
template <typename Dtype>
class MinihandSampleTransformer {
 public:
  // 构造函数
  explicit MinihandSampleTransformer(const MinihandTransformationParameter& param, Phase phase)
      : param_(param), phase_(phase) {}
  virtual ~MinihandSampleTransformer() {}

  // anno -> 输入标注 
  // image -> 返回图像
  // bboxes -> 返回的Gt-Boxes
  // bool -> 处理成功，返回true，否则返回false
  bool Transform(HandAnnoData<Dtype>& anno, cv::Mat* image, vector<LabeledBBox<Dtype> >* bboxes, cv::Mat& bg_image);
  // 处理裁剪问题
