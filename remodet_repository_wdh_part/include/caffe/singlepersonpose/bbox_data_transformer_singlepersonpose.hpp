#ifndef CAFFE_MASK_BBOX_DATA_TRANSFORMER_SINGLEPERSONPOSE_HPP_
#define CAFFE_MASK_BBOX_DATA_TRANSFORMER_SINGLEPERSONPOSE_HPP_
...
class BBoxDataSinglePersonPoseTransformer {
 public:
  // 构造函数
  // 我们仍然使用UnifiedTransformationParameter进行构造
  explicit BBoxDataSinglePersonPoseTransformer(const UnifiedTransformationParameter& param, Phase phase)
      : param_(param), phase_(phase) { InitRand(); }
  virtual ~BBoxDataSinglePersonPoseTransformer() {}
  ...
  void TransformMetaPoints(AnnoData<Dtype>& anno);
  ...
  void get_mask(AnnoData<Dtype>& anno, cv::Mat& image,Dtype* transformed_heatmap,Dtype* transformed_vecmap);
