...
int main(int nargc, char** args) {
  ...
  layer_param.set_name("DepthwiseConv2DNHWC");
  layer_param.set_type("ConvolutionDepthwiseNHWC");
  ...
  boost::shared_ptr<Layer<float> > DepthwiseConv2DNHWC_layer = LayerRegistry<float>::CreateLayer(layer_param);
  DepthwiseConv2DNHWC_layer->SetUp(bottom_vec, top_vec);
  DepthwiseConv2DNHWC_layer->Forward(bottom_vec, top_vec);

  GradientChecker<float> checker(1e-4, 1e-3);
  checker.CheckGradientExhaustive(DepthwiseConv2DNHWC_layer.get(), bottom_vec, top_vec);
