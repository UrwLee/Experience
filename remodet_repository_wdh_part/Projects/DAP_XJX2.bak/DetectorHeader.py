...
def UnitLayerDetectorHeader(net, data_layer="data", num_classes=2, feature_layer="conv5", \
        ...
        flat=False, use_focus_loss=False, stage=1,lr_mult=1, decay_mult=1):
    ...
    if use_focus_loss:
        ConvBNUnitLayer(net, from_layer, name, use_bn=False, use_relu=False,num_output=num_conf_output, kernel_size=3, \
             pad=1, stride=1,init_xavier=False,bias_type='focal',sparse=num_classes,lr_mult=lr_mult, decay_mult=decay_mult)
    else:
        ConvBNUnitLayer(net, from_layer, name, use_bn=False, use_relu=False, \
...
def MultiLayersDenseDetectorHeader(net, data_layer="data", num_classes=2, from_layers=[], \
        normalizations=[], use_batchnorm=True, prior_variance = [0.1], \
        pro_widths=[], pro_heights=[], flip=True, clip=True, \
        inter_layer_channels=[], use_focus_loss=False, stage=1,lr_mult=1, decay_mult=1):
    assert num_classes, "must provide num_classes"
    assert num_classes > 0, "num_classes must be positive number"
    if normalizations:
        assert len(from_layers) == len(normalizations), "from_layers and normalizations should have same length"
...
def SsdDetectorHeaders(net, net_width=300, net_height=300, data_layer="data", \
            ...
            use_dense_boxes=False, stage=1,lr_mult=1, decay_mult=1):
