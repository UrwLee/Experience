...
def DAPNet_Train():
	...
	net = DAPNetVGGReduceEltWise(net, train=True, data_layer="data", gt_label="label",\
	...
	if isinstance(val_list, list):
		...
		for id_val in xrange(len(val_list)):
			net = caffe.NetSpec()
			net = get_DAPDataLayer(net, train=False, batchsize=1, id_val=id_val)
			test_net_file = "{}/test{}.prototxt".format(proto_dir, id_val)
			net = DAPNetDarkPoseEltWise(net, train=False, data_layer="data", gt_label="label", \
										  net_width=resized_width, net_height=resized_height)
