...
sys.path.insert(0, '/home/xjx/work/remodet_repository_DJ')
sys.path.remove('/home/xjx/work/remodet_repository/python')
# sys.path.remove('/home/xjx/work/remodet_repository')
...
def Deconv(net,from_layer,num_output,group,kernel_size,stride,lr_mult,decay_mult,use_bn,use_scale,use_relu,add_str = ""):
    deconv_param = {
...
def DetRelease_FirstBodyPartPoseNet(train=True):
    if train:
        ...
        ##Step1: Create Data for Body_Part Detection of 16:9, 9:16 and Pose Estimation
        net = get_DAPDataLayer(net, train=train, batchsize=batch_size,data_name = "data",label_name = "label",flag_169=True)
        net = get_DAPDataLayer(net, train=train, batchsize=batch_size, data_name="data_288x512", label_name="label_288x512", flag_169=False)
        ...
        net = addconv6(net, from_layer=from_layer, use_bn=use_bn, conv6_output=conv6_output, \
                       conv6_kernal_size=conv6_kernal_size, pre_name="conv6", start_pool=False, lr_mult=lr_mult,
                       decay_mult=1, n_group=1, flag_withparamname=True)
        ...
        net = addconv6(net, from_layer=from_layer, use_bn=use_bn, conv6_output=conv6_output, post_name="_288x512",
                       conv6_kernal_size=conv6_kernal_size, pre_name="conv6", start_pool=False, lr_mult=lr_mult,
                       decay_mult=decay_mult, n_group=1, flag_withparamname=True)
        ##Step4:Create featuremap1,featuremap2,featuremap3 for Detection subnet_16:9
        ...
        gt_label = "label" + add_str
        net_width = 512
        net_height = 288
        flag_169 = True
        ssd_Param_1 = get_ssd_Param_1(flag_169=flag_169,bboxloss_loc_weight = 2.5,bboxloss_conf_weight=2.5)
        mbox_1_layers = SsdDetectorHeaders(net, \
                                           ...
                                           stage=1, lr_mult=lr_mult,flag_withparamname=True,add_str=add_str)
        ...
        net["mbox_1_loss" + add_str] = L.BBoxLoss(*mbox_1_layers, bbox_loss_param=bboxloss_param, \
                                            loss_param=loss_param, include=dict(phase=caffe_pb2.Phase.Value('TRAIN')), \
                                            propagate_down=[True, True, False, False])

        ##Step7:Create Header and Part Loss for  subnet_16:9
        ssd_Param_2 = get_ssd_Param_2(flag_169=flag_169, bboxloss_loc_weight=2.0 / 4.0, bboxloss_conf_weight=1.0 / 4.0)
        mbox_2_layers = SsdDetectorHeaders(net, \
                                           ...
                                           stage=2,flag_withparamname=True,add_str=add_str)
        ...
        use_scale = False
        use_relu = False
        out_layer1 = Deconv(net, from_layer, num_output, group, kernel_size, stride, lr_mult, decay_mult, use_bn, use_scale,use_relu,add_str)
        from_layer = "conv4_5"
        ...
        ConvBNUnitLayer(net, from_layer + add_str, out_layer2, use_bn=use_bn, use_relu=False,
                        ...
                        leaky=False, lr_mult=lr_mult,decay_mult=decay_mult)
        feat_layers = []
        ...
        for stage in xrange(use_stage):
            ...
            net = mPose_StageX_Train(net, from_layer=from_layer, out_layer=outlayer, stage=stage + 1,
                                     ...
                                     base_layer=baselayer, lr=lr_mult, decay=decay_mult, num_channels=n_channel,
                                     kernel_size=kernel_size, flag_sigmoid=flag_output_sigmoid,loss_weight=0.1)

    else:
        ##Step1: Create Data for Body_Part Detection of 16:9, 9:16
        ##Step2: Create BaseNet for two subnets until conv5_5
        ##Step3: Create Conv6 for Body_Part Detection for Detection subnets(16:9 and 9:16)
        ##Step4: Create featuremap1,featuremap2,featuremap3 for Detection subnet_16:9
        ##Step5: Create featuremap1,featuremap2,featuremap3 for Detection subnet_9:16
        ##Step6: Create Body Header and sigmoid conf for subnet_16:9
        ##Step7: Create Part Header and sigmoid conf for subnet_16:9
        ##Step8: Create evaluation part for subnet_16:9
        ##Step9: Create Body Header and sigmoid conf for subnet_9:16
        ##Step10:Create Part Header and sigmoid conf for subnet_9:16
        ##Step11:Create evaluation part for subnet_9:16
        net1 = caffe.NetSpec()
        net2 = caffe.NetSpec()
        ##Step1: Create Data for Body_Part Detection of 16:9, 9:16
        net1 = get_DAPDataLayer(net1, train=train, batchsize=batch_size, data_name="data", label_name="label",
                               flag_169=True)
        net2 = get_DAPDataLayer(net2, train=train, batchsize=batch_size, data_name="data_288x512",
                               label_name="label_288x512", flag_169=False)
        ...
        kernels = ((3,), (3,), (3, 1, 3), (3, 1, 3, 1, 3), (3, 1, 3, 1, 3))
        pool_last = (False, False, False, True, True)
        net1 = VGG16_BaseNet_ChangeChannel(net1, from_layer="data", channels=channels, strides=strides,
                                          kernels=kernels, freeze_layers=[], pool_last=pool_last,
                                          flag_withparamname=True, add_string='',
                                          use_bn=use_bn, lr_mult=lr_mult, decay_mult=decay_mult, use_global_stats=None)
        net2 = VGG16_BaseNet_ChangeChannel(net2, from_layer="data_288x512", channels=channels, strides=strides,
                                          kernels=kernels, freeze_layers=[], pool_last=pool_last,
                                          flag_withparamname=True,
                                          add_string='_288x512', use_bn=use_bn, lr_mult=lr_mult, decay_mult=decay_mult,
                                          use_global_stats=None)
        ##Step3: Create Conv6 for Body_Part Detection for Detection subnets(16:9 and 9:16)
        lr_mult = 1.0
        conv6_output = Conv6_Param.get('conv6_output', [])
        conv6_kernal_size = Conv6_Param.get('conv6_kernal_size', [])
        from_layer = "pool5"
        net1 = addconv6(net1, from_layer=from_layer, use_bn=use_bn, conv6_output=conv6_output, \
                       conv6_kernal_size=conv6_kernal_size, pre_name="conv6", start_pool=False, lr_mult=lr_mult,
                       decay_mult=1, n_group=1, flag_withparamname=True)
        from_layer = "pool5_288x512"
        net2 = addconv6(net2, from_layer=from_layer, use_bn=use_bn, conv6_output=conv6_output, post_name="_288x512",
                       conv6_kernal_size=conv6_kernal_size, pre_name="conv6", start_pool=False, lr_mult=lr_mult,
                       decay_mult=1, n_group=1, flag_withparamname=True)
        ##Step4:Create featuremap1,featuremap2,featuremap3 for Detection subnet_16:9
        layers = ["conv3_3", "conv4_5"]
        ...
        num_channels = 128
        add_str = ""
        MultiScaleEltLayer(net1, layers=layers, kernels=kernels, strides=strides, out_layer=out_layer,
                           num_channels=num_channels, lr=1.0, decay=1.0, use_bn=use_bn, add_str=add_str,
                           flag_withparamname=True)
        layers = ["conv4_5", "conv5_5"]
        kernels = [3, 3]
        ...
        num_channels = 128
        add_str = "_288x512"
        MultiScaleEltLayer(net2, layers=layers, kernels=kernels, strides=strides, out_layer=out_layer,
                           num_channels=num_channels, lr=1.0, decay=1.0, use_bn=use_bn, add_str=add_str,
                           flag_withparamname=True)
        layers = ["conv4_5", "conv5_5"]
        kernels = [3, 3]
        ...
        flag_169 = True
        ssd_Param_1 = get_ssd_Param_1(flag_169=flag_169, bboxloss_loc_weight=2.5, bboxloss_conf_weight=2.5)
        mbox_1_layers = SsdDetectorHeaders(net1, \
                                           net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                           from_layers=ssd_Param_1.get('feature_layers', []), \
                                           num_classes=ssd_Param_1.get("num_classes", 2), \
                                           boxsizes=ssd_Param_1.get("anchor_boxsizes", []), \
        ...
        if ssd_Param_1.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.SOFTMAX:
            ...
            net1[reshape_name] = L.Reshape(mbox_1_layers[1], \
                                          shape=dict(dim=[0, -1, ssd_Param_1.get("num_classes", 2)]))
            softmax_name = "mbox_1_conf_softmax" + add_str
            net1[softmax_name] = L.Softmax(net1[reshape_name], axis=2)
            flatten_name = "mbox_1_conf_flatten" + add_str
            net1[flatten_name] = L.Flatten(net1[softmax_name], axis=1)
            mbox_1_layers[1] = net1[flatten_name]
            ...
            net1[sigmoid_name] = L.Sigmoid(mbox_1_layers[1])
            mbox_1_layers[1] = net1[sigmoid_name]
        else:
        ...
        net1["detection_out_1" + add_str] = L.DetOut(*mbox_1_layers, \
                                                    detection_output_param=det_out_param, \
                                                    include=dict(phase=caffe_pb2.Phase.Value('TEST')))
        ##Step7:Create Part Header and sigmoid conf for subnet_16:9
        ssd_Param_2 = get_ssd_Param_2(flag_169=flag_169, bboxloss_loc_weight=2.0 / 4.0, bboxloss_conf_weight=1.0 / 4.0)
        mbox_2_layers = SsdDetectorHeaders(net1, \
                                           net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                           from_layers=ssd_Param_2.get('feature_layers', []), \
                                           num_classes=ssd_Param_2.get("num_classes", 2), \
                                           boxsizes=ssd_Param_2.get("anchor_boxsizes", []), \
        ...
        if ssd_Param_2.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.SOFTMAX:
            ...
            net1[reshape_name] = L.Reshape(mbox_2_layers[1], \
            ...
            net1[flatten_name] = L.Flatten(net1[softmax_name], axis=1)
            mbox_2_layers[1] = net1[flatten_name]
            ...
            net1[sigmoid_name] = L.Sigmoid(mbox_2_layers[1])
            mbox_2_layers[1] = net1[sigmoid_name]
        else:
            raise ValueError("Unknown conf loss type.")
        det_out_param = {
            'num_classes': ssd_Param_2.get("num_classes", 2),
        ...
        net1["detection_out_2" + add_str] = L.DenseDetOut(*mbox_2_layers, \
                                                         detection_output_param=det_out_param, \
                                                         include=dict(phase=caffe_pb2.Phase.Value('TEST')))
        ##Step8:Create evaluation part for subnet_16:9
        ...
        det_out_layers = []
        det_out_layers.append(net1['detection_out_1' + add_str])
        det_out_layers.append(net1['detection_out_2' + add_str])
        name = 'det_out' + add_str
        net1[name] = L.Concat(*det_out_layers, axis=2)
        net1["det_accu" + add_str] = L.DetEval(net1[name], net1[gt_label], \
        ...
        flag_169 = False
        ssd_Param_1 = get_ssd_Param_1(flag_169=flag_169, bboxloss_loc_weight=2.5, bboxloss_conf_weight=2.5)
        mbox_1_layers = SsdDetectorHeaders(net2, \
                                           net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                           from_layers=ssd_Param_1.get('feature_layers', []), \
                                           num_classes=ssd_Param_1.get("num_classes", 2), \
                                           boxsizes=ssd_Param_1.get("anchor_boxsizes", []), \
        ...
        if ssd_Param_1.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.SOFTMAX:
            reshape_name = "mbox_1_conf_reshape" + add_str
            net2[reshape_name] = L.Reshape(mbox_1_layers[1], \
                                          shape=dict(dim=[0, -1, ssd_Param_1.get("num_classes", 2)]))
            softmax_name = "mbox_1_conf_softmax" + add_str
            net2[softmax_name] = L.Softmax(net2[reshape_name], axis=2)
            flatten_name = "mbox_1_conf_flatten" + add_str
            net2[flatten_name] = L.Flatten(net2[softmax_name], axis=1)
            mbox_1_layers[1] = net2[flatten_name]
        elif ssd_Param_1.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.LOGISTIC:
            sigmoid_name = "mbox_1_conf_sigmoid" + add_str
            net2[sigmoid_name] = L.Sigmoid(mbox_1_layers[1])
            mbox_1_layers[1] = net2[sigmoid_name]
        else:
            raise ValueError("Unknown conf loss type.")
        det_out_param = {
            'num_classes': ssd_Param_1.get("num_classes", 2),
        ...
        net2["detection_out_1" + add_str] = L.DetOut(*mbox_1_layers, \
                                                    detection_output_param=det_out_param, \
                                                    include=dict(phase=caffe_pb2.Phase.Value('TEST')))
        ##Step10:Create Part Header and sigmoid conf for subnet_9:16
        ssd_Param_2 = get_ssd_Param_2(flag_169=flag_169, bboxloss_loc_weight=2.0 / 4.0, bboxloss_conf_weight=1.0 / 4.0)
        mbox_2_layers = SsdDetectorHeaders(net2, \
                                           net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                           from_layers=ssd_Param_2.get('feature_layers', []), \
                                           num_classes=ssd_Param_2.get("num_classes", 2), \
                                           boxsizes=ssd_Param_2.get("anchor_boxsizes", []), \
        ...
        if ssd_Param_2.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.SOFTMAX:
            reshape_name = "mbox_2_conf_reshape" + add_str
            net2[reshape_name] = L.Reshape(mbox_2_layers[1], \
                                          shape=dict(dim=[0, -1, ssd_Param_2.get("num_classes", 2)]))
            softmax_name = "mbox_2_conf_softmax" + add_str
            net2[softmax_name] = L.Softmax(net2[reshape_name], axis=2)
            flatten_name = "mbox_2_conf_flatten" + add_str
            net2[flatten_name] = L.Flatten(net2[softmax_name], axis=1)
            mbox_2_layers[1] = net2[flatten_name]
        elif ssd_Param_2.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.LOGISTIC:
            sigmoid_name = "mbox_2_conf_sigmoid" + add_str
            net2[sigmoid_name] = L.Sigmoid(mbox_2_layers[1])
            mbox_2_layers[1] = net2[sigmoid_name]
        else:
            raise ValueError("Unknown conf loss type.")
        det_out_param = {
            'num_classes': ssd_Param_2.get("num_classes", 2),
        ...
        net2["detection_out_2" + add_str] = L.DenseDetOut(*mbox_2_layers, \
                                                         detection_output_param=det_out_param, \
                                                         include=dict(phase=caffe_pb2.Phase.Value('TEST')))
        ##Step11:Create evaluation part for subnet_9:16
        eval_Param = get_eval_Param([0, 1, 3])
        ...
        det_out_layers = []
        det_out_layers.append(net2['detection_out_1' + add_str])
        det_out_layers.append(net2['detection_out_2' + add_str])
        name = 'det_out' + add_str
        net2[name] = L.Concat(*det_out_layers, axis=2)
        net2["det_accu" + add_str] = L.DetEval(net2[name], net2[gt_label], \
                                              detection_evaluate_param=det_eval_param, \
                                              include=dict(phase=caffe_pb2.Phase.Value('TEST')))
        net = [net1,net2]
    return net
...
def DetRelease_SecondPartAllNet(train=True):
    if train:
        ...
        net = VGG16_BaseNet_ChangeChannel(net, from_layer="data_minihand", channels=channels, strides=strides,
                                          kernels=kernels, freeze_layers=[], pool_last=pool_last, flag_withparamname=True,
                                          add_string='_minihand',use_bn=use_bn, lr_mult=lr_mult, decay_mult=decay_mult, use_global_stats=None)
        ##Step3: Create Conv6 for Body_Part Detection for Detection subnets(16:9 and 9:16)
        lr_mult = 0.0
        conv6_output = Conv6_Param.get('conv6_output',[])
        conv6_kernal_size = Conv6_Param.get('conv6_kernal_size',[])
        ...
        add_str = ""
        MultiScaleEltLayer(net, layers=layers, kernels=kernels, strides=strides, out_layer=out_layer,
                           num_channels=num_channels, lr=lr_mult, decay=1.0, use_bn=use_bn, add_str=add_str,
                           flag_withparamname=True)
        layers = ["conv4_5", "conv5_5"]
        kernels = [3, 3]
        strides = [2, 1]
        ...
        ##Step7:Create Header and Part Loss for  subnet_16:9
        ssd_Param_2 = get_ssd_Param_2(flag_169=flag_169_global, bboxloss_loc_weight=2.0, bboxloss_conf_weight=1.0)
        mbox_2_layers = SsdDetectorHeaders(net, \
                                           net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                           from_layers=ssd_Param_2.get('feature_layers', []), \
                                           num_classes=ssd_Param_2.get("num_classes", 2), \
        ...
        Deconv(net, from_layer, num_output=64, group=1, kernel_size=2, stride=2, lr_mult=1.0, decay_mult=1.0,
               use_bn=use_bn, use_scale=use_bn, use_relu=False,add_str="_minihand")

        out_layer = "hand_multiscale"
        net[out_layer] = L.Eltwise(net["conv2_hand"], net["conv4_5_Upsample"],
                                   eltwise_param=dict(operation=P.Eltwise.SUM))
        from_layer = out_layer
        out_layer = from_layer + "_relu"
        net[out_layer] = L.ReLU(net[from_layer], in_place=True)
        data_layer = "data_minihand"
        gt_label = "label_minihand"

        ssd_Param_3 = get_ssd_Param_3(flag_169_global, bboxloss_loc_weight=2.0, bboxloss_conf_weight=1.0)
        ...
        net = caffe.NetSpec()
        ##Step1: Create Data for Body_Part Detection of 16:9, 9:16
        net = get_DAPDataLayer(net, train=train, batchsize=batch_size, data_name="data", label_name="label",
        ...
        if flag_169_global:
            ...
            net_height = 512
        ##Step7:Create Part Header and sigmoid conf for subnet_16:9
        ssd_Param_2 = get_ssd_Param_2(flag_169=flag_169_global, bboxloss_loc_weight=2.0 / 4.0, bboxloss_conf_weight=1.0 / 4.0)
        mbox_2_layers = SsdDetectorHeaders(net, \
                                           net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                           from_layers=ssd_Param_2.get('feature_layers', []), \
                                           num_classes=ssd_Param_2.get("num_classes", 2), \
