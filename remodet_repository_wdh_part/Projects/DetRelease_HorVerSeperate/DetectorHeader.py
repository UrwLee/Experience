...
def MultiLayersDetectorHeader(net, data_layer="data", num_classes=2, from_layers=[], \
        ...
        pro_widths=[], pro_heights=[], flip=True, clip=True, inter_layer_channels=[], \
        use_focus_loss=False, stage=1,lr_mult=1.0,decay_mult=1.0,flag_withparamname=False,flagcreateprior=True,add_str = ""):
    assert num_classes, "must provide num_classes"
    assert num_classes > 0, "num_classes must be positive number"
    if normalizations:
    ...
    for i in range(0, num):
        ...
        loc_layer,conf_layer,priorbox_layer = \
            UnitLayerDetectorHeader(net, data_layer=data_layer, num_classes=num_classes, \
                ...
                flat=True, use_focus_loss=use_focus_loss, stage=stage,lr_mult=lr_mult,decay_mult=decay_mult,flag_withparamname=flag_withparamname,flagcreateprior=flagcreateprior,
                                    add_str=add_str)
...
def SsdDetectorHeaders(net, net_width=300, net_height=300, data_layer="data", \
            ...
            use_dense_boxes=False, stage=1,lr_mult=1.0,decay_mult=1.0,flag_withparamname=False,flagcreateprior=True,add_str = ""):
    ...
    if use_dense_boxes:
        ...
        mbox_layers = MultiLayersDetectorHeader(net, data_layer=data_layer, num_classes=num_classes, \
            ...
            prior_variance = prior_variance, pro_widths=pro_widths, pro_heights=pro_heights, \
            flip=flip, clip=clip, inter_layer_channels=inter_layer_channels, \
            use_focus_loss=use_focus_loss, stage=stage,lr_mult=lr_mult,decay_mult=decay_mult,flag_withparamname=flag_withparamname,flagcreateprior=flagcreateprior,add_str=add_str)
    return mbox_layers
