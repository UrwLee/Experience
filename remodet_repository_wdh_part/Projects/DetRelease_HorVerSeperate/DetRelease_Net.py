...
def DetRelease_FirstBodyPartPoseNet(train=True):
    ...
    mbox_1_layers = SsdDetectorHeaders(net, \
                                       ...
                                       stage=1, lr_mult=lr_inter_loss,flag_withparamname=False,add_str=add_str)
    ##Step7:Create Header and Part Loss for  subnet_16:9
...
def DetRelease_SecondPartAllNetMiniHandFace(train=True):
    ...
    if train:
        ...
        net["det_accu"] = L.DetEval(net["detection_out_2"], net[gt_label], \
                                               ...
                                               include=dict(phase=caffe_pb2.Phase.Value('TEST')))
    return net
def DetRelease_SecondPartHandFaceTwoSeperateBBoxLoss(train=True):
    ...
    ##Step7:Create Header and Part Loss for  subnet_16:9
    ssd_Param_allhand = get_ssd_Param_allhand(flag_169=flag_169_global, bboxloss_loc_weight=bboxloss_loc_weight_part, bboxloss_conf_weight=bboxloss_conf_weight_part)
    mbox_2_layers = SsdDetectorHeaders(net, \
                                       net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                       from_layers=ssd_Param_allhand.get('feature_layers', []), \
                                       num_classes=ssd_Param_allhand.get("num_classes", 2), \
                                       boxsizes=ssd_Param_allhand.get("anchor_boxsizes", []), \
                                       aspect_ratios=ssd_Param_allhand.get("anchor_aspect_ratios", []), \
                                       prior_variance=ssd_Param_allhand.get("anchor_prior_variance", [0.1, 0.1, 0.2, 0.2]), \
                                       flip=ssd_Param_allhand.get("anchor_flip", True), \
                                       clip=ssd_Param_allhand.get("anchor_clip", True), \
                                       normalizations=ssd_Param_allhand.get("interlayers_normalizations", []), \
                                       use_batchnorm=ssd_Param_allhand.get("interlayers_use_batchnorm", True), \
                                       inter_layer_channels=ssd_Param_allhand.get("interlayers_channels_kernels", []), \
                                       use_focus_loss=ssd_Param_allhand.get("bboxloss_using_focus_loss", False), \
                                       use_dense_boxes=ssd_Param_allhand.get('bboxloss_use_dense_boxes', False), \
                                       stage=2,flag_withparamname=False,add_str=add_str,lr_mult=lr_inter_loss)
    feature_layers = []
    slice_layers = ["featuremap1_2_inter_2", "featuremap2_2_inter_2", "featuremap3_2_inter_2"]
    if train:
        for s_layer in slice_layers:
            net[s_layer + "_face"],net[s_layer + "_mini"]= \
            L.Slice(net[s_layer], ntop=2, slice_param=dict(slice_point=[batch_size], axis=0))
            net[s_layer + "_silence"] = L.Silence(net[s_layer + "_mini"], ntop=0)
            feature_layers.append(s_layer + "_face")
    else:
        for s_layer in slice_layers:
            out_layer = s_layer + "_face"
            from_layer = [net[s_layer],]
            net[out_layer] = L.Concat(*from_layer, axis=0)
            feature_layers.append(out_layer)

    ssd_Param_face = get_ssd_Param_facenointerlayer(flag_169=flag_169_global,
                                                        bboxloss_loc_weight=bboxloss_loc_weight_part,
                                                        bboxloss_conf_weight=bboxloss_conf_weight_part,
                                                        feature_layers=feature_layers)
    print(ssd_Param_face.get("interlayers_channels_kernels", []),"ssd_Param_face.get()")
    mbox_3_layers = SsdDetectorHeaders(net, \
                                       net_width=net_width, net_height=net_height, data_layer=data_layer, \
                                       from_layers=ssd_Param_face.get('feature_layers', []), \
                                       num_classes=ssd_Param_face.get("num_classes", 2), \
                                       boxsizes=ssd_Param_face.get("anchor_boxsizes", []), \
                                       aspect_ratios=ssd_Param_face.get("anchor_aspect_ratios", []), \
                                       prior_variance=ssd_Param_face.get("anchor_prior_variance",
                                                                            [0.1, 0.1, 0.2, 0.2]), \
                                       flip=ssd_Param_face.get("anchor_flip", True), \
                                       clip=ssd_Param_face.get("anchor_clip", True), \
                                       normalizations=ssd_Param_face.get("interlayers_normalizations", []), \
                                       use_batchnorm=ssd_Param_face.get("interlayers_use_batchnorm", True), \
                                       inter_layer_channels=ssd_Param_face.get("interlayers_channels_kernels", []), \
                                       use_focus_loss=ssd_Param_face.get("bboxloss_using_focus_loss", False), \
                                       use_dense_boxes=ssd_Param_face.get('bboxloss_use_dense_boxes', False), \
                                       flagcreateprior=ssd_Param_face.get('flagcreateprior', True), \
                                       stage=3, flag_withparamname=False, add_str=add_str, lr_mult=lr_inter_loss)
    priorbox_layers = []
    priorbox_layers.append(net["featuremap1_2_mbox_priorbox"])
    priorbox_layers.append(net["featuremap2_2_mbox_priorbox"])
    priorbox_layers.append(net["featuremap3_2_mbox_priorbox"])

    name = "mbox_{}_priorbox".format(3)
    net[name] = L.Concat(*priorbox_layers, axis=2)
    mbox_3_layers.append(net[name])


    if train:
        gt_label = "label"
        loss_param = get_loss_param(normalization=ssd_Param_allhand.get("bboxloss_normalization", P.Loss.VALID))
        mbox_2_layers.append(net[gt_label])

        bboxloss_param = {
            'gt_labels': ssd_Param_allhand.get('gt_labels', []),
            'target_labels': ssd_Param_allhand.get('target_labels', []),
            'num_classes': ssd_Param_allhand.get("num_classes", 2),
            'alias_id': ssd_Param_allhand.get("alias_id", 0),
            'loc_loss_type': ssd_Param_allhand.get("bboxloss_loc_loss_type", P.MultiBoxLoss.SMOOTH_L1),
            'conf_loss_type': ssd_Param_allhand.get("bboxloss_conf_loss_type", P.MultiBoxLoss.LOGISTIC),
            'loc_weight': ssd_Param_allhand.get("bboxloss_loc_weight", 1),
            'conf_weight': ssd_Param_allhand.get("bboxloss_conf_weight", 1),
            'overlap_threshold': ssd_Param_allhand.get("bboxloss_overlap_threshold", 0.5),
            'neg_overlap': ssd_Param_allhand.get("bboxloss_neg_overlap", 0.5),
            'size_threshold': ssd_Param_allhand.get("bboxloss_size_threshold", 0.0001),
            'do_neg_mining': ssd_Param_allhand.get("bboxloss_do_neg_mining", True),
            'neg_pos_ratio': ssd_Param_allhand.get("bboxloss_neg_pos_ratio", 3),
            'using_focus_loss': ssd_Param_allhand.get("bboxloss_using_focus_loss", False),
            'gama': ssd_Param_allhand.get("bboxloss_focus_gama", 2),
            'use_difficult_gt': ssd_Param_allhand.get("bboxloss_use_difficult_gt", False),
            'code_type': ssd_Param_allhand.get("bboxloss_code_type", P.PriorBox.CENTER_SIZE),
            'use_prior_for_matching': True,
            'encode_variance_in_target': False,
            'flag_noperson': ssd_Param_allhand.get('flag_noperson', False),
        }
        net["mbox_2_loss" + add_str] = L.BBoxLoss(*mbox_2_layers, bbox_loss_param=bboxloss_param, \
                                             loss_param=loss_param,
                                             include=dict(phase=caffe_pb2.Phase.Value('TRAIN')), \
                                             propagate_down=[True, True, False, False])
        gt_label = "label_pd"
        loss_param = get_loss_param(normalization=ssd_Param_face.get("bboxloss_normalization", P.Loss.VALID))
        mbox_3_layers.append(net[gt_label])

        bboxloss_param = {
            'gt_labels': ssd_Param_face.get('gt_labels', []),
            'target_labels': ssd_Param_face.get('target_labels', []),
            'num_classes': ssd_Param_face.get("num_classes", 2),
            'alias_id': ssd_Param_face.get("alias_id", 0),
            'loc_loss_type': ssd_Param_face.get("bboxloss_loc_loss_type", P.MultiBoxLoss.SMOOTH_L1),
            'conf_loss_type': ssd_Param_face.get("bboxloss_conf_loss_type", P.MultiBoxLoss.LOGISTIC),
            'loc_weight': ssd_Param_face.get("bboxloss_loc_weight", 1),
            'conf_weight': ssd_Param_face.get("bboxloss_conf_weight", 1),
            'overlap_threshold': ssd_Param_face.get("bboxloss_overlap_threshold", 0.5),
            'neg_overlap': ssd_Param_face.get("bboxloss_neg_overlap", 0.5),
            'size_threshold': ssd_Param_face.get("bboxloss_size_threshold", 0.0001),
            'do_neg_mining': ssd_Param_face.get("bboxloss_do_neg_mining", True),
            'neg_pos_ratio': ssd_Param_face.get("bboxloss_neg_pos_ratio", 3),
            'using_focus_loss': ssd_Param_face.get("bboxloss_using_focus_loss", False),
            'gama': ssd_Param_face.get("bboxloss_focus_gama", 2),
            'use_difficult_gt': ssd_Param_face.get("bboxloss_use_difficult_gt", False),
            'code_type': ssd_Param_face.get("bboxloss_code_type", P.PriorBox.CENTER_SIZE),
            'use_prior_for_matching': True,
            'encode_variance_in_target': False,
            'flag_noperson': ssd_Param_face.get('flag_noperson', False),
        }
        net["mbox_3_loss" + add_str] = L.BBoxLoss(*mbox_3_layers, bbox_loss_param=bboxloss_param, \
                                                  loss_param=loss_param,
                                                  include=dict(phase=caffe_pb2.Phase.Value('TRAIN')), \
                                                  propagate_down=[True, True, False, False])
        ...
        if ssd_Param_allhand.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.SOFTMAX:
            reshape_name = "mbox_2_conf_reshape" + add_str
            net[reshape_name] = L.Reshape(mbox_2_layers[1], \
                                          shape=dict(dim=[0, -1, ssd_Param_allhand.get("num_classes", 2)]))
            softmax_name = "mbox_2_conf_softmax" + add_str
            net[softmax_name] = L.Softmax(net[reshape_name], axis=2)
            flatten_name = "mbox_2_conf_flatten" + add_str
            net[flatten_name] = L.Flatten(net[softmax_name], axis=1)
            mbox_2_layers[1] = net[flatten_name]
        elif ssd_Param_allhand.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.LOGISTIC:
            sigmoid_name = "mbox_2_conf_sigmoid" + add_str
            net[sigmoid_name] = L.Sigmoid(mbox_2_layers[1])
            mbox_2_layers[1] = net[sigmoid_name]
        else:
        ...
        det_out_param = {
            'num_classes': ssd_Param_allhand.get("num_classes", 2),
            'target_labels': ssd_Param_allhand.get('detout_target_labels', []),
            'alias_id': ssd_Param_allhand.get("alias_id", 0),
            'conf_threshold': ssd_Param_allhand.get("detout_conf_threshold", 0.01),
            'nms_threshold': ssd_Param_allhand.get("detout_nms_threshold", 0.45),
            'size_threshold': ssd_Param_allhand.get("detout_size_threshold", 0.0001),
            'top_k': ssd_Param_allhand.get("detout_top_k", 30),
            'share_location': True,
            'code_type': P.PriorBox.CENTER_SIZE,
            'background_label_id': 0,
            'variance_encoded_in_target': False,
        ...
        net["detection_out_2"] = L.DetOut(*mbox_2_layers, \
                                                         detection_output_param=det_out_param, \
                                                         include=dict(phase=caffe_pb2.Phase.Value('TEST')))
        if ssd_Param_face.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.SOFTMAX:
            reshape_name = "mbox_3_conf_reshape" + add_str
            net[reshape_name] = L.Reshape(mbox_3_layers[1], \
                                          shape=dict(dim=[0, -1, ssd_Param_face.get("num_classes", 2)]))
            softmax_name = "mbox_3_conf_softmax" + add_str
            net[softmax_name] = L.Softmax(net[reshape_name], axis=2)
            flatten_name = "mbox_3_conf_flatten" + add_str
            net[flatten_name] = L.Flatten(net[softmax_name], axis=1)
            mbox_3_layers[1] = net[flatten_name]
        elif ssd_Param_face.get("bboxloss_conf_loss_type", P.MultiBoxLoss.SOFTMAX) == P.MultiBoxLoss.LOGISTIC:
            sigmoid_name = "mbox_3_conf_sigmoid" + add_str
            net[sigmoid_name] = L.Sigmoid(mbox_3_layers[1])
            mbox_3_layers[1] = net[sigmoid_name]
        else:
        ...
        det_out_param = {
            'num_classes': ssd_Param_face.get("num_classes", 2),
            'target_labels': ssd_Param_face.get('detout_target_labels', []),
            'alias_id': ssd_Param_face.get("alias_id", 0),
            'conf_threshold': ssd_Param_face.get("detout_conf_threshold", 0.01),
            'nms_threshold': ssd_Param_face.get("detout_nms_threshold", 0.45),
            'size_threshold': ssd_Param_face.get("detout_size_threshold", 0.0001),
            'top_k': ssd_Param_face.get("detout_top_k", 30),
            'share_location': True,
            'code_type': P.PriorBox.CENTER_SIZE,
            'background_label_id': 0,
            'variance_encoded_in_target': False,
        ...
        net["detection_out_3"] = L.DetOut(*mbox_3_layers, \
                                                         detection_output_param=det_out_param, \
                                                         include=dict(phase=caffe_pb2.Phase.Value('TEST')))
