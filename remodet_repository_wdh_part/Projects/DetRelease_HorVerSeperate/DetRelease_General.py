...
"""
train_net_id = 1
flag_169_global = True#
flag_noperson_body = True
if train_net_id == 1:
    flag_noperson_part = True #if train_net_id=0,default: False; if train_net_id=1, defaut: True.
else:
    flag_noperson_part = False
...
bboxloss_neg_pos_ratio_part = 20
###mini
bboxloss_overlap_threshold_mini = 0.5
bboxloss_neg_overlap_mini = 0.3
bboxloss_neg_pos_ratio_mini = 20
flag_miniresizeddata = True
flag_mininetwithface = False #Default: false; if true: minihandata has face; if false: minihanndata only has hand
flag_handfacetwoboxloss = True #default: false
...
if train_net_id == 0:
    ...
    batch_size_pose = 12#default:12
    bboxloss_loc_weight_body = 3#default:4.0
    ...
    if flag_mininetwithface:
        ...
        if flag_handfacetwoboxloss:
            base_bindex = 0
            base_bindex_mini = batch_size
        else:
            base_bindex = 0
            base_bindex_mini = 0
...
if train_net_id == 0:#train Body and Part
    ...
    Models = "%s_BDPDBBoxS%sDistP%s%s%s_BD%sLWLoc%sConf%sPIOU%sNIOU%sR%d_PDDenseBBoxLoc%sConf%sPosIOU%sNegIOU%sR%d"\
             %(model_pre,get_scale_str(min_scale,max_scale),str(dist_prob),noperson_str,data_str_body,bdloss_str,str(bboxloss_loc_weight_body),str(bboxloss_conf_weight_body),
    ...
    if not flag_mininetwithface:
        if flag_handfacetwoboxloss:
            Models = "%s_PDBBoxS%s%sMinihand%s%s%sDistP%s_FaceBBoxLoc%sConf%sPIOU%sNIOU%sR%d_HandBBoxLoc%sConf%sPIOU%sNIOU%sR%d" \
            % (model_pre, get_scale_str(min_scale, max_scale), data_str_part, mini_str, data_str_mini, noperson_str,
               str(dist_prob), str(bboxloss_loc_weight_part), str(bboxloss_conf_weight_part),
               str(bboxloss_overlap_threshold_part), str(bboxloss_neg_overlap_part), bboxloss_neg_pos_ratio_part,
               str(bboxloss_loc_weight_part), str(bboxloss_conf_weight_part), str(bboxloss_overlap_threshold_mini),
               str(bboxloss_neg_overlap_mini), bboxloss_neg_pos_ratio_mini)
        else:
            Models = "%s_PDBBoxS%s%sMinihand%s%s%sDistP%s_PDDensBBoxLoc%sConf%sPIOU%sNIOU%sR%d_MiniBBoxLoc%sConf%sPIOU%sNIOU%sR%d" \
                 % (model_pre, get_scale_str(min_scale,max_scale), data_str_part,mini_str,data_str_mini,noperson_str,str(dist_prob),str(bboxloss_loc_weight_part), str(bboxloss_conf_weight_part),
                    str(bboxloss_overlap_threshold_part), str(bboxloss_neg_overlap_part),bboxloss_neg_pos_ratio_part,
