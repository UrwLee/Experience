...
# ---------------------------------SSD1 Params----------------------------------
# Parameters to Generate Body_Header
...
# ##############################################################################
# ---------------------------------SSD2 Params----------------------------------
# Parameters to Generate Part_Header
...
# Parameters to Generate Minihand_Header
...
# Parameters to Generate Part_And_Minihand_Header
...
# Parameters to Generate hand including all scales
def get_ssd_Param_allhand(flag_169,bboxloss_loc_weight = 2.0,bboxloss_conf_weight=1.0):
    anchor_boxsizes_allhand = [[0.03,],]
    anchor_boxsizes_allhand.extend(anchor_boxsizes_part)
    asize_all = []
    for a in anchor_boxsizes_allhand:
        asize_all.extend(a)
    if flag_169:
        resized_width = 512
        resized_height = 288
        ...
        resized_width = 288
        resized_height = 512
    asize_all = tuple(asize_all)
    anchor_aspect_pixels_allhand = {
        asize_all: (1,),
    }
    anchor_aspect_ratios_allhand = get_anchor_aspect_ratios(anchor_boxsizes_allhand, anchor_aspect_pixels_allhand,
    ...
    return {
        ...
        'num_classes': 2,
        'gt_labels': gt_labels,
        'target_labels': getTargetLabels(len(gt_labels)),
        'alias_id': gt_labels[0],
        # Anchors
        'anchor_boxsizes': anchor_boxsizes_allhand,
        # 'anchor_aspect_ratios': ,
        'anchor_aspect_ratios': anchor_aspect_ratios_allhand,
        'anchor_prior_variance': [0.1, 0.1, 0.2, 0.2],
        'anchor_flip': True,
        'anchor_clip': True,
        # InterLayers
        ...
        'bboxloss_normalization': P.Loss.VALID,
        # detection out
        'detout_target_labels': gt_labels,
        'detout_conf_threshold': 0.5,
        'detout_nms_threshold': 0.4,
        'detout_size_threshold': 0.0001,
        'detout_top_k': 200,
...
# Parameters to Generate hand including all scales
def get_ssd_Param_facenointerlayer(flag_169,bboxloss_loc_weight = 2.0,bboxloss_conf_weight=1.0,feature_layers=[]):
    gt_labels = [3,]
    return {
...
print get_ssd_Param_facenointerlayer(True)