...
void MinihandDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  minihand_transformer_.reset(
     new MinihandSampleTransformer<Dtype>(minihand_transform_param_, this->phase_));
  // 获取数据Layer参数
  ...
  LOG(INFO) << "output data size: " << top[0]->num() << ","
    ...
    << top[0]->width();
  // label
  ndim_label_ = minihand_data_param.ndim_label();
...
//    readBgImg(bg_img);
    ...
    vector<LabeledBBox<Dtype> > bboxes; // func: 
    bool flag = minihand_transformer_->Transform(anno, &image, &bboxes, bg_img);
    if (! flag) { // 增广失败
      item_id--;  // 回滚到下一条记录
    } else {
...
template <typename Dtype>
void MinihandDataLayer<Dtype>::setupBg(){
    bg_lines_id_ = 0;
...
*/
template <typename Dtype>
void MinihandDataLayer<Dtype>::readBgImg(cv::Mat& bg_img){
  string xml_root = bg_lines_[bg_lines_id_].first;
  string xml_path = xml_root + '/' + bg_lines_[bg_lines_id_].second;
  HandAnnoData<Dtype> bg_anno;
  ReadHandDataFromXml(0, xml_path, xml_root, &bg_anno);

  bg_img = cv::imread(bg_anno.image_path.c_str());
  if (!bg_img.data)  LOG(FATAL) << "Error when open image file: " << bg_anno.image_path;
  bg_lines_id_ ++;
