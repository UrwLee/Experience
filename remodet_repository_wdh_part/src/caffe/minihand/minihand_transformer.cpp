...
bool MinihandTransformer<Dtype>::Transform(HandAnnoData<Dtype>& anno, cv::Mat* image, vector<LabeledBBox<Dtype> >* bboxes, cv::Mat& bg_image) {
  // (0) 如果没有实例，直接返回
...
bool MinihandTransformer<Dtype>::getCropBBox(HandAnnoData<Dtype>& anno, BoundingBox<Dtype>* crop_roi, Phase phase) {
  ...
  if(param_.sample_sixteennine()){ // func: 面积扩大两倍的　16:9 背景
    rw = int((temp * 2) / 16) * 16; // scale area 2 #16:9
    rh = rw / 16 * 9;//16:9
  }else if(param_.sample_ninesixteen()){
    rw = int((temp * 2) / 9) * 9; // scale area 2 #9:16
    rh = rw / 9 * 16;//9:16
  }
  // } else {
...
*/
template <typename Dtype>
void MinihandTransformer<Dtype>::borderImage(cv::Mat& image){
  /*
...
*/
template <typename Dtype>
void MinihandTransformer<Dtype>::unNormalize(HandAnnoData<Dtype>& anno) {
 //
  ...
  for (it = anno.hands.begin(); it != anno.hands.end();) {
    ...
    if(( xmin >= xmax ) || ( ymin >= ymax ) || (gt_bbox.compute_area() <= 0)) {
...
*/
template <typename Dtype>
void MinihandTransformer<Dtype>::RelatGtBbox(HandAnnoData<Dtype> anno, BoundingBox<Dtype>& relat_gt_bbox){
...
*/
template <typename Dtype>
void MinihandTransformer<Dtype>::changeInstance(HandAnnoData<Dtype> anno, HandAnnoData<Dtype>& temp_anno, 
...
*/
template <typename Dtype>
void MinihandTransformer<Dtype>::cutGtImgChangeBg(cv::Mat raw_image, cv::Mat& bg_img, HandAnnoData<Dtype> anno, HandAnnoData<Dtype> temp_anno){
  for(int i=0; i < anno.hands.size(); ++i){
...
*/
template <typename Dtype>
void MinihandTransformer<Dtype>::normRelatGtBox(BoundingBox<Dtype>& temp_box, BoundingBox<Dtype> box){
...
*/
template <typename Dtype>
void MinihandTransformer<Dtype>::instancesFitBg(cv::Size shape_bg_img, BoundingBox<Dtype>& relat_gt_bbox, 
...
template <typename Dtype>
void MinihandTransformer<Dtype>::cutGtChangeBg(HandAnnoData<Dtype>& anno , cv::Mat& raw_image, cv::Mat& bg_img){
  // 如果 超过概率
  if (false) { return; }
