...
template <typename Dtype>
__global__ void im2col_nhwc_gpu_kernel(const int n, const Dtype* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    ...
    const int w_col = (index / channels) % width_col;
    const int h_col = index / channels / width_col;
    const int h_offset = h_col * stride_h - pad_h;
    const int w_offset = w_col * stride_w - pad_w;
    const int c_channel = (h_col * width_col + w_col) * kernel_h * kernel_w * channels;
    Dtype* data_col_ptr = data_col + c_channel + c_im;
    const Dtype* data_im_ptr = data_im + c_im;
    for (int i = 0; i < kernel_h; ++i) {
      for (int j = 0; j < kernel_w; ++j) {
        int h_im = h_offset + i * dilation_h;
        int w_im = w_offset + j * dilation_w;
        *data_col_ptr =
            (h_im >= 0 && w_im >= 0 && h_im < height && w_im < width) ?
            data_im_ptr[(h_im * width + w_im) * channels] : 0;
...
template <typename Dtype>
void im2col_nhwc_gpu(const Dtype* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w,
  ...
  int width_col = (width + 2 * pad_w -
      (dilation_w * (kernel_w - 1) + 1)) / stride_w + 1;
  int num_kernels = channels * height_col * width_col;
  im2col_nhwc_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),CAFFE_CUDA_NUM_THREADS>>>(
...
template void im2col_nhwc_gpu<float>(const float* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w, float* data_col);
template void im2col_nhwc_gpu<double>(const double* data_im, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h, const int stride_w,
    const int dilation_h, const int dilation_w, double* data_col);
...
// NOTE: it's original implementation method.
template <typename Dtype>
__global__ void col2im_nhwc_gpu_kernel(const int n, const Dtype* data_col,
    const int height, const int width, const int channels,
    const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w,
    const int stride_h, const int stride_w,
  ...
  CUDA_KERNEL_LOOP(index, n) {
    Dtype val = 0;
    const int c_im = index % channels;
    const int w_im = (index / channels) % width + pad_w;
    const int h_im = index / channels / width + pad_h;
    int kernel_extent_w = (kernel_w - 1) * dilation_w + 1;
    int kernel_extent_h = (kernel_h - 1) * dilation_h + 1;
    // compute the start and end of the output
    const int w_col_start =
    ...
    for (int h_col = h_col_start; h_col < h_col_end; h_col += 1) {
      for (int w_col = w_col_start; w_col < w_col_end; w_col += 1) {
        ...
        if (h_k % dilation_h == 0 && w_k % dilation_w == 0) {
          h_k /= dilation_h;
          w_k /= dilation_w;
          int data_col_index = (((h_col * width_col + w_col) * kernel_h + h_k) * kernel_w + w_k) * channels + c_im;
          val += data_col[data_col_index];
        }
      }
    }
...
// NOTE: it's another implementation method.
// template <typename Dtype>
// __global__ void col2im_nhwc_gpu_kernel(const int n, const Dtype* data_col,
//     const int height, const int width, const int channels,
//     const int kernel_h, const int kernel_w,
...
//   CUDA_KERNEL_LOOP(index, n) {
//     const int c_im = index % channels;
//     const int k_w = (index / channels) % kernel_w;
//     const int k_h = (index / channels / kernel_w) % kernel_h;
//     const int o_w = (index / channels / kernel_w / kernel_h) % width_col;
//     const int o_h = index / channels / kernel_w / kernel_h / width_col;
//     // input dim
//     const int i_h = -pad_h + o_h * stride_h + k_h * dilation_h;
//     const int i_w = -pad_w + o_w * stride_w + k_w * dilation_w;
//
//     if (i_h >= 0 && i_w >= 0 && i_h < height && i_w < width) {
//       const int stored_idx = (((i_h * width + i_w) * channels + c_im) * kernel_h + k_h) * kernel_w + k_w;
//       buf[stored_idx] = data_col[index];
//     }
//   }
// }

template <typename Dtype>
void col2im_nhwc_gpu(const Dtype* data_col, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w,
    Dtype* data_im) {
  ...
  int width_col = (width + 2 * pad_w - (dilation_w * (kernel_w - 1) + 1)) /
      stride_w + 1;
  int num_kernels = channels * height * width;
  col2im_nhwc_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(num_kernels),CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, data_col, height, width, channels, kernel_h, kernel_w,
      pad_h, pad_w, stride_h, stride_w, dilation_h, dilation_w,
      height_col, width_col, data_im);
  // NOTE: it's another implementation method.
  // int n = height_col * width_col * kernel_h * kernel_w * channels;
  // int n_buf = height * width * channels * kernel_h * kernel_w;
  // vector<int> buf_shape(1,n_buf);
  // Blob<Dtype> diff_buf(buf_shape);
  // vector<int> kernel_dim_shape(1, kernel_h * kernel_w);
  // Blob<Dtype> kernel_multiplier(kernel_dim_shape);
  // caffe_gpu_set(diff_buf.count(), Dtype(0), diff_buf.mutable_gpu_data());
  // caffe_gpu_set(kernel_multiplier.count(), Dtype(1), kernel_multiplier.mutable_gpu_data());
  // col2im_nhwc_gpu_kernel<Dtype><<<CAFFE_GET_BLOCKS(n),CAFFE_CUDA_NUM_THREADS>>>(
  //     n, data_col, height, width, channels, kernel_h, kernel_w, pad_h, pad_w,
  //     stride_h, stride_w, dilation_h, dilation_w, height_col, width_col,
  //     diff_buf.mutable_gpu_data());
  // caffe_gpu_gemv<Dtype>(CblasNoTrans, height * width * channels, kernel_h * kernel_w, 1.,
  //     diff_buf.gpu_data(), kernel_multiplier.gpu_data(), 0., data_im);
  CUDA_POST_KERNEL_CHECK;
}

template void col2im_nhwc_gpu<float>(const float* data_col, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w,
    float* data_im);
template void col2im_nhwc_gpu<double>(const double* data_col, const int channels,
    const int height, const int width, const int kernel_h, const int kernel_w,
    const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w,
    double* data_im);
