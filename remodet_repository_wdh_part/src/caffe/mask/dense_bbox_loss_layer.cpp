...
void DenseBBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  bool flag_ignore_gt_ = dense_bbox_loss_param.flag_ignore_gt();
  if(flag_ignore_gt_){
    CHECK_GT(ndim_label, 9)<< "Label needs to provide ignore gt label";
  }
  ...
  //   LOG(INFO)<<
  // }
  int cnt = 0; // Nouse; Only for Debugging. 有多少个无gt image
  if (flag_noperson_){
  ...
  for (int i = 0; i < num_; ++i) {
    ...
    // 匹配
    // visualize(gt_bboxes, 1);
    if (flag_ignore_gt_){
      ExhaustMatchAnchorsAndGTsIgnoreGt(gt_bboxes, prior_bboxes, overlap_threshold_, neg_overlap_,
                        &match_indices, &unmatch_indices, flags_of_anchor_,flag_noperson_,flag_forcematchallgt_,size_threshold_[1]);
    }
    else {
      if(flag_areamaxcheckinmatch_){  // default: true
   ...
   //################################################################Draw Matched Priorbox Start
    /*
      将匹配的prior box 输出, 显示prior box 匹配效果; 
    */
    ...
    //     char imagename [256];
    //     sprintf(imagename, "/home/xjx/REMO_HandPose_20180912_DetHand_DataGroup/vis_match/augment_%06d.jpg", counter);
    ...
    if (flag_noperson_){ // set true
      ...
      if (false) {
        ...
        // }
        // for (int j = 0; j < num_pos_; j++) {
        //   pos_loc_boxes_batch.push_back(prior_bboxes[match_indices_vec[j]]);
        // }
        for (int j = 0; j < num_neg_; j++) {
          neg_loc_boxes_batch.push_back(prior_bboxes[unmatch_indices[j]]);
        }

        visualize(pos_loc_boxes_batch, 2);
        visualize(neg_loc_boxes_batch, 3);
      }
...
if(flag_noperson_){
  int num_neg_all = scores_indices.size(); 
   //NOTE: vector<pair<Dtype, pair<int, int> > > scores_indices;
  // NOTE: scores_indices  pair <score, pari<prior-id, batch-id> >
...
template<typename Dtype>
void DenseBBoxLossLayer<Dtype>::visualize(vector<LabeledBBox<Dtype> > boxes, int flag) {
  ...
  char filename[100];
  cv::Mat image;
  sprintf(filename, "/home/xjx/img_vis/img_raw/augment_%06d.jpg", counter);
  // LOG(INFO) << filename;
  image = cv::imread(filename);//导入图片

  cv::Mat img_vis = image.clone();

  for (int i = 0; i < boxes.size(); ++i) {
    LabeledBBox<Dtype>& box = boxes[i];
    BoundingBox<Dtype>& tbox = box.bbox;
    BoundingBox<Dtype> bbox_real;
    bbox_real.x1_ = tbox.x1_ * 512;//img_vis.cols;
    bbox_real.y1_ = tbox.y1_ * 288;//img_vis.rows;
    bbox_real.x2_ = tbox.x2_ * 512;//img_vis.cols;
    bbox_real.y2_ = tbox.y2_ * 288;//img_vis.rows;

    bbox_real.Draw( 0 , 191 , 255, &img_vis);
  }
  char imagename [256];
  if (flag == 1) {
    sprintf(imagename, "%s/augment_%06d.jpg", "/home/xjx/img_vis/gt", counter);
    // counter++;
  }
  else if (flag == 2) {
    sprintf(imagename, "%s/augment_%06d.jpg", "/home/xjx/img_vis/pos", counter);
    // counter++;
  }
  else if (flag == 3) {
    // sprintf(imagename, "%s/augment_%06d.jpg", "/home/xjx/awdh_old/cppCode/alike_layer/neg_0.1_0.3", counter);
    // sprintf(imagename, "%s/augment_%06d.jpg", "/home/xjx/awdh_old/cppCode/alike_layer/neg_0.1", counter);
    // sprintf(imagename, "%s/augment_%06d.jpg", "/home/xjx/awdh_old/cppCode/alike_layer/neg_0.2~0.3", counter);
    // sprintf(imagename, "%s/augment_%06d.jpg", "/home/xjx/awdh_old/cppCode/alike_layer/neg0.3", counter);
    sprintf(imagename, "%s/augment_%06d.jpg", "/home/xjx/img_vis/neg", counter);
    counter++;
  }
