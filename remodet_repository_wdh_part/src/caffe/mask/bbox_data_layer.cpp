...
BBoxDataLayer<Dtype>::~BBoxDataLayer<Dtype>() { // func: 开启线程
...
template <typename Dtype>
void BBoxDataLayer<Dtype>::setupBg(){
    bg_lines_id_ = 0;
...
*/
template <typename Dtype>
void BBoxDataLayer<Dtype>::readBgImg(cv::Mat& bg_img){
...
void BBoxDataLayer<Dtype>::ReadAnnoDataFromXml(const int bindex, const string& xml_file, const string& root_dir,
  ...
  // for(int j=0; j<anno->instances.size(); ++j){
  //   Instance<Dtype> ins2 = anno->instances[j];
  //   LOG(INFO) << "xxxxxxxx  " << ins2.bb;
  // }
}
