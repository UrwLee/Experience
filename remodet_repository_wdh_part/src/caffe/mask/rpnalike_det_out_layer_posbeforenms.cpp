...
#include "caffe/mask/rpnalike_det_out_layer_posebeforenms.hpp"
#include "caffe/util/myimg_proc.hpp"

namespace caffe {

template <typename Dtype>
void RPNAlikeDetOutBeforeNMSLayer<Dtype>::LayerSetUp(
  ...
  num_positive_max_ = detection_output_param.num_positive_max();
}

template <typename Dtype>
void RPNAlikeDetOutBeforeNMSLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void RPNAlikeDetOutBeforeNMSLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
  ...
  for (int i = 0; i < num; ++i) {
    ...
    MatchAnchorsAndGTs(gt_bboxes, loc_bboxes_filtersize, match_type, overlap_threshold_, neg_overlap_,
  ...
  //LOG(INFO)<<" match_indices_vec "<<match_indices_vec.size()<<" gt_bboxes "<<gt_bboxes.size()<<" unmatch_indices "<<unmatch_indices.size();
    ...
    if(match_indices_vec.size()>num_positive_max_){
      num_pos = num_positive_max_;
      random_shuffle(match_indices_vec.begin(),match_indices_vec.end());
    } 
    for (int j=0;j<num_pos;j++){
        pos_loc_boxes_batch.push_back(loc_bboxes_filtersize[match_indices_vec[j]]);
    }
    num_pos_ += num_pos;
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(RPNAlikeDetOutBeforeNMSLayer, Forward);
#endif

INSTANTIATE_CLASS(RPNAlikeDetOutBeforeNMSLayer);
REGISTER_LAYER_CLASS(RPNAlikeDetOutBeforeNMS);
