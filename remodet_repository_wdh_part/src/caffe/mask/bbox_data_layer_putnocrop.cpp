...
#include "caffe/mask/bbox_data_layer_putnocrop.hpp"
...
BBoxDataPutNoCropLayer<Dtype>::~BBoxDataPutNoCropLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void BBoxDataPutNoCropLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  bbox_data_transformer_.reset(
     new BBoxDataTransformerPutNoCropLayer<Dtype>(bbox_data_transform_param_, this->phase_));
  // 获取数据Layer参数
  ...
  if (unified_data_param.rand_skip()) {
    ...
    lines_id_ = skip;//０到rang_skip中随机找个数
  }
  // data
...
template <typename Dtype>
void BBoxDataPutNoCropLayer<Dtype>::ShuffleLists() {
...
template <typename Dtype>
void BBoxDataPutNoCropLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    //LOG(INFO)<<"xml_path:"<<xml_path;
    // Read Anno
    AnnoData<Dtype> anno;
    // std::cout << "Error when open image file: " << xml_path;
    ...
    for (int s = 0; s < unified_data_param.mean_value_size(); ++s) {
      mean_values.push_back(unified_data_param.mean_value(s));//3个mean
    }
...
template <typename Dtype>
void BBoxDataPutNoCropLayer<Dtype>::ReadAnnoDataFromXml(const int bindex, const string& xml_file, const string& root_dir,
                                                  AnnoData<Dtype>* anno) {
  ptree pt;
  read_xml(xml_file, pt);
  //LOG(INFO)<<"img_name:"<<pt.get<string>("Annotations.ImagePath");
...
INSTANTIATE_CLASS(BBoxDataPutNoCropLayer);
REGISTER_LAYER_CLASS(BBoxDataPutNoCrop);
}  // namespace caffe
