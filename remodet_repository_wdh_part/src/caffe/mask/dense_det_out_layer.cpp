...
#include "caffe/mask/dense_det_out_layer.hpp"
#include "caffe/util/myimg_proc.hpp"

namespace caffe {

template <typename Dtype>
void DenseDetOutLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void DenseDetOutLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void DenseDetOutLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
  const DetectionOutputParameter &detection_output_param =
      this->layer_param_.detection_output_param();
  ...
  获取LOC的估计信息
  #***************************************************************************/
  vector<vector<LabeledBBox<Dtype> > > all_loc_preds; // shape: [num][c-1][Np]
  GetLocPreds(loc_data, num, num_priors * (num_classes_ - 1), &all_loc_preds);
  /**************************************************************************#
  获取每个部位的置信度信息
  #***************************************************************************/
  vector<vector<vector<Dtype> > > all_conf_scores(num); // shape: [num][c][Np]
  // 统计所有类别的置信度信息　[包含bkg]
  ...
  #***************************************************************************/
  // [num,class,prior_id]
  vector<vector<vector<LabeledBBox<Dtype> > > > all_decode_bboxes; // shape: [num][c-1][Np]
  ...
  #***************************************************************************/
  int num_det = 0;
  vector<vector<vector<LabeledBBox<Dtype> > > > all_dets(num); // shape: [num][c-1][np] 其中没有0 类 背景类
  vector<vector<int> > prior_ids;
  prior_ids.resize(num);
  const bool use_voting = true;
  for (int i = 0; i < num; ++i) {
    for (int j = 1; j < num_classes_; ++j) { // num_class 从1 开始, 1, 2
      vector<int> indices;
      if (use_voting) {
        const Dtype voting_thre = 0.75;
        if (false){
            NmsFastWithVotingChangeScore(&all_decode_bboxes[i][j-1], &all_conf_scores[i][j], conf_threshold_,
                          nms_threshold_, top_k_, voting_thre, &indices); // box 的类是从0开始, 0, 1 是c-1,  conf是从0开始 , 0,1,2 , 0是背景类
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(DenseDetOutLayer, Forward);
#endif

INSTANTIATE_CLASS(DenseDetOutLayer);
REGISTER_LAYER_CLASS(DenseDetOut);
