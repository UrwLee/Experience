...
#include "caffe/mask/bbox_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void BBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void BBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void BBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  num_ = bottom[0]->num();//batch_size
  num_priors_ = bottom[2]->height() / 4;//anchor的数量
  num_gt_ = bottom[3]->height();//gt数量
  /**************************************************************************#
  获取整个batch的GT-boxes
  #***************************************************************************/
 ...
 //  LOG(INFO)<<cnt<< " images for batchsizie "<<num_<<" In bbox_loss_layer";
  ...
  for (int i = 0; i < num_; ++i) {
    ...
    // ============  可视化 prior box =======================
    // cv::Mat gt_image(512, 288, CV_8UC3, cv::Scalar(0,0,0) );
    // //vector<LabeledBBox<Dtype> > prior_bboxes;
    // for (map<int,int>::iterator it = match_indices.begin();
    //     it != match_indices.end(); ++it) {
    //     BoundingBox<Dtype> prior_box = prior_bboxes[it->first].bbox;
    //     // cv::Mat gt_image = cv::imread("");
    //     cv::Point pt1 = cv::Point((int)prior_box.x1_, (int)prior_box.y1_);
    //     cv::Point pt2 = cv::Point((int)prior_box.x2_, (int)prior_box.y2_);
    //     cv::rectangle(gt_image, pt1, pt2, cv::Scalar(0,128,255), 1);
    //     LOG(INFO) << pt1.x << ", "<< pt1.y;
    //     LOG(INFO) << pt2.x << ", "<< pt2.y;
    // }
    // cv::imshow("img", gt_image);
...
template <typename Dtype>
void BBoxLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_CLASS(BBoxLossLayer);
REGISTER_LAYER_CLASS(BBoxLoss);
