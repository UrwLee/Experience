...
#include "caffe/reid/unlabeled_match_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void UnlabeledMatchLayer<Dtype>::LayerSetUp(
      const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  queue_size_ = this->layer_param_.unlabeled_match_param().queue_size();
  queue_tail_ = 0;
...
template <typename Dtype>
void UnlabeledMatchLayer<Dtype>::Reshape(
      const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void UnlabeledMatchLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  // num
  ...
  // num of stored unlabeled instances
...
template <typename Dtype>
void UnlabeledMatchLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(UnlabeledMatchLayer);
#endif

INSTANTIATE_CLASS(UnlabeledMatchLayer);
REGISTER_LAYER_CLASS(UnlabeledMatch);
