...
void DetectionMultiMcOutputLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
  const DetectionOutputParameter &detection_output_param =
      this->layer_param_.detection_output_param();
  CHECK(detection_output_param.has_num_classes()) << "Must specify num_classes.";
  num_classes_ = detection_output_param.num_classes() - 1;
...
template <typename Dtype>
void DetectionMultiMcOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void DetectionMultiMcOutputLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(DetectionMultiMcOutputLayer, Forward);
#endif

INSTANTIATE_CLASS(DetectionMultiMcOutputLayer);
REGISTER_LAYER_CLASS(DetectionMultiMcOutput);
