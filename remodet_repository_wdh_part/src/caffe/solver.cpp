...
// 两种Solver 的初始化方式, 直接读取 solver param, 
template <typename Dtype>
Solver<Dtype>::Solver(const SolverParameter& param, const Solver* root_solver)
    : net_(), callbacks_(), root_solver_(root_solver), 
      requested_early_exit_(false) {
    // net_ type:shared_ptr<Net<Dtype> > , callbacks_ type: vector<Callback*>, root_solver_ type const Solver* const
    // requested_early_exit_ 收到停止信息为true
  Init(param);
}
...
void Solver<Dtype>::Init(const SolverParameter& param) {
  CHECK(Caffe::root_solver() || root_solver_) // 检查是否 root_solver_ 为根solver , root_solver 哪里设置
      << "root_solver_ needs to be set for all non-root solvers";
  LOG_IF(INFO, Caffe::root_solver()) << "Initializing solver from parameters: "
    << std::endl << param.DebugString();
  param_ = param; // 
  CHECK_GE(param_.average_loss(), 1) << "average_loss should be non-negative."; 
  CheckSnapshotWritePermissions(); // 检查快照地址, 并且写一个临时文件测试
  if (Caffe::root_solver() && param_.random_seed() >= 0) { // 设置随机种子 
    Caffe::set_random_seed(param_.random_seed());
  }
  // Scaffolding code
  InitTrainNet(); // 读取train net 的param, 初始化net 的stage,  创建net_ 指针
...
template <typename Dtype>
void Solver<Dtype>::InitTrainNet() { // net, net_param, OR train_net, train_net_param, 只能有一对
  const int num_train_nets = param_.has_net() + param_.has_net_param() + // solver param 中可以训练多个net
      param_.has_train_net() + param_.has_train_net_param(); 
      // LOG_IF(INFO, Caffe::root_solver()) << "===== "<< num_train_nets << ", " << param_.has_net() << ", " << param_.has_net_param()<< ", " <<param_.has_train_net() << ", " <<param_.has_train_net_param();
  ...
  if (param_.has_net()) { // 不直接读取 net param, 而是从文件中读取 ,读取train net 信息
    LOG_IF(INFO, Caffe::root_solver())
...
void Solver<Dtype>::CheckSnapshotWritePermissions() { 
  if (Caffe::root_solver() && param_.snapshot()) { // 是否设置 快照次数, 每5000次进行一次快照
    CHECK(param_.has_snapshot_prefix()) // 是否设置 快照的 保存路径
        << "In solver params, snapshot is specified but snapshot_prefix is not";
    string probe_filename = SnapshotFilename(".tempfile"); // 创造临时 快照文件
...
template <typename Dtype>
string Solver<Dtype>::SnapshotFilename(const string extension) { // set extension = ".tempfile"
  return param_.snapshot_prefix() + "_iter_" + caffe::format_int(iter_) // 根据iter_设置路径名称
    + extension;
}
