 ...
 CUDA_KERNEL_LOOP(index,nthreads) {
   ...
   if (x_on_map >= 0 && x_on_map < oriSpatialWidth && y_on_map >= 0 && y_on_map < oriSpatialHeight){
     ...
     linear_interpolation(&v,
                          src_ptr[(c*oriSpatialHeight+y_nei[0])*oriSpatialWidth+x_nei[0]],
                          src_ptr[(c*oriSpatialHeight+y_nei[0])*oriSpatialWidth+x_nei[1]],
                          src_ptr[(c*oriSpatialHeight+y_nei[1])*oriSpatialWidth+x_nei[0]],
                          src_ptr[(c*oriSpatialHeight+y_nei[1])*oriSpatialWidth+x_nei[1]],
                          dx, dy);
     dst_ptr[index] = v;
   } else {
     dst_ptr[index] = 0;
...
template <typename Dtype>
void RoiSResizeLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){
...
template <typename Dtype>
void RoiSResizeLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  return;
}

INSTANTIATE_LAYER_GPU_FUNCS(RoiSResizeLayer);
