...
#include "caffe/mask/bbox_data_addpose_layer.hpp"
...
BBoxDataAddPoseLayer<Dtype>::~BBoxDataAddPoseLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void BBoxDataAddPoseLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  bbox_data_transformer_.reset(
     new BBoxDataAddPoseTransformer<Dtype>(bbox_data_transform_param_, this->phase_));
  // 获取数据Layer参数
  ...
  int num_limbs=17;
  int num_parts=18;
  top[1]->Reshape(1,1,1,9+batch_size*(4*num_limbs+2*num_parts)*channelOffset);
  for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
    this->prefetch_[i].label_.Reshape(1,1,1,9+batch_size*(4*num_limbs+2*num_parts)*channelOffset);
  }
...
template <typename Dtype>
void BBoxDataAddPoseLayer<Dtype>::ShuffleLists() {
...
template <typename Dtype>
void BBoxDataAddPoseLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  // Dtype* top_labels = batch->label_.mutable_cpu_data();
    ...
    int num_limbs=17;
    int num_parts=18;
    // batch->label_.Reshape(1,1,1,9+batch_size*(4*num_limbs+2*num_parts)*channelOffset);
      Blob<Dtype> blob_tmp;
    blob_tmp.Reshape(1,1,1,batch_size*(4*num_limbs+2*num_parts)*channelOffset);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    int old=(4*num_limbs+2*num_parts)*item_id;
    Dtype* transformed_vec_mask = blob_tmp.mutable_cpu_data()+old*channelOffset;
    Dtype* transformed_heat_mask =blob_tmp.mutable_cpu_data()+ (2*num_limbs+old)*channelOffset;
    Dtype* transformed_vecmap = blob_tmp.mutable_cpu_data() + (2*num_limbs+num_parts+old)*channelOffset;
    Dtype* transformed_heatmap = blob_tmp.mutable_cpu_data() + (4*num_limbs+num_parts+old)*channelOffset;

    if (anno.num_person == 0 && this->phase_ == TRAIN){
      bbox_data_transformer_->Transform(anno, &image, &doflip,&maskmiss);    
    } else{
      bbox_data_transformer_->Transform(anno, &image, &bboxes, &crop_bbox, &doflip,&maskmiss,transformed_vec_mask,transformed_heat_mask,transformed_vecmap,transformed_heatmap);
      num_p += bboxes.size();
      bboxes_all.push_back(bboxes);

    // for(int t=0;t<50;++t) LOG(INFO)<<transformed_vec_mask[t]<<"after!";
    // for(int t=0;t<500;++t) LOG(INFO)<<transformed_heatmap[t]<<"############";
    ...
    //   vector<BBoxDataAddPose<Dtype> > part_bboxes;
  ...
  batch->label_.Reshape(1,1,1,num_gt*9+batch_size*(4*num_limbs+2*num_parts)*channelOffset);
    Dtype* top_pose = batch->label_.mutable_cpu_data();
caffe_copy<Dtype>(batch_size*(4*num_limbs+2*num_parts)*channelOffset, blob_tmp.mutable_cpu_data(),top_pose);
...
//else LOG(INFO)<<t;
 ...
 // caffe_copy<Dtype>(batch_size*(4*num_limbs+2*num_parts)*channelOffset, blob_tmp.mutable_cpu_data(),top_pose);
  Dtype* top_label = batch->label_.mutable_cpu_data()+batch_size*(4*num_limbs+2*num_parts)*channelOffset;
  int count = 0;
  int idx = 0;
  for (int i = 0; i < bboxes_all.size(); ++i) {
...
template <typename Dtype>
void BBoxDataAddPoseLayer<Dtype>::ReadAnnoDataFromXml(const int bindex, const string& xml_file, const string& root_dir,
                                                  AnnoData<Dtype>* anno) {
  ptree pt;
  ...
  for (int i = 0; i < anno->num_person; ++i) {
    ...
    try {
      ...
      ins.joint.joints.resize(18);
      ins.joint.isVisible.resize(18);
      for(int j = 0; j < 18; ++j) {
...
INSTANTIATE_CLASS(BBoxDataAddPoseLayer);
REGISTER_LAYER_CLASS(BBoxDataAddPose);
}  // namespace caffe
