...
#include "caffe/mask/bbox_data_transformer_multi_resize_batch.hpp"
#include "caffe/mask/bbox_data_multi_resize_batch_layer.hpp"
...
BBoxDataMultiResizeBatchLayer<Dtype>::~BBoxDataMultiResizeBatchLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void BBoxDataMultiResizeBatchLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  bbox_data_transformer_.reset(
     new BBoxDataMultiResizeBatchTransformer<Dtype>(bbox_data_transform_param_, this->phase_));
  bbox_data_transformer_->fetch_ratio_id_ = 0;
  // 获取数据Layer参数
...
template <typename Dtype>
void BBoxDataMultiResizeBatchLayer<Dtype>::ShuffleLists() {
...
template <typename Dtype>
void BBoxDataMultiResizeBatchLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  CHECK(batch->data_.count());
  const UnifiedDataParameter& unified_data_param = this->layer_param_.unified_data_param();
  // string xml_root = unified_data_param.xml_root();
  const int batch_size = unified_data_param.batch_size();
  const int height = bbox_data_transform_param_.resized_height_repeat(bbox_data_transformer_->fetch_ratio_id_);
  const int width = bbox_data_transform_param_.resized_width_repeat(bbox_data_transformer_->fetch_ratio_id_);
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (anno.num_person == 0 && this->phase_ == TRAIN){
      ...
      bbox_data_transformer_->Transform(anno, &image, &bboxes, &crop_bbox, &doflip);
      num_p += bboxes.size();
  ...
  bbox_data_transformer_->fetch_ratio_id_++;
  if (bbox_data_transformer_->fetch_ratio_id_ == bbox_data_transform_param_.resized_height_repeat_size()){
    bbox_data_transformer_->fetch_ratio_id_ = 0;
  }
  // check data
   ...
   // LOG(INFO)<<image.rows<<"*"<<height<<"~"<<image.cols<<"*"<<width;
    ...
    for (int y = 0; y < image.rows; ++y) {
      for (int x = 0; x < image.cols; ++x) {
        ...
        if (normalize) {
          top_data_item[y * width + x] = (rgb[0] - 128)/256.0;
          top_data_item[offset + y * width + x] = (rgb[1] - 128)/256.0;
          top_data_item[2 * offset + y * width + x] = (rgb[2] - 128)/256.0;
        } else {
          top_data_item[y * width + x] = rgb[0] - mean_values_[0];
          top_data_item[offset + y * width + x] = rgb[1] - mean_values_[1];
          top_data_item[2 * offset + y * width + x] = rgb[2] - mean_values_[2];
        }
      }
    }
  }
...
template <typename Dtype>
void BBoxDataMultiResizeBatchLayer<Dtype>::ReadAnnoDataFromXml(const int bindex, const string& xml_file, const string& root_dir,
                                                  AnnoData<Dtype>* anno) {
  ptree pt;
...
INSTANTIATE_CLASS(BBoxDataMultiResizeBatchLayer);
REGISTER_LAYER_CLASS(BBoxDataMultiResizeBatch);
}  // namespace caffe
