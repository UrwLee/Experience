...
#include "caffe/mask/bbox_data_fs_layer.hpp"
...
BBoxDataFsLayer<Dtype>::~BBoxDataFsLayer<Dtype>() {
  this->StopInternalThread();
}

template <typename Dtype>
void BBoxDataFsLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (unified_data_param.has_xml_list()) {
    ...
    for (int i = 0; i < unified_data_param.xml_list_multiple_size(); i++) {
      ...
      while (infile >> xmlname) {
        ptree pt;
        string xml_file = xml_root + '/' + xmlname;
        read_xml(xml_file, pt);
        int num_person = pt.get<int>("Annotations.NumPerson");
        if(num_person == 0){
          neg_lines_.push_back(make_pair(xml_root, xmlname));
        } else{
            pos_lines_.push_back(make_pair(xml_root, xmlname));
  ...
  max_neg_num_ = unified_data_param.max_neg_num();
  ...
  CHECK(!pos_lines_.empty()) << "File pos is empty.";
  CHECK(!neg_lines_.empty()) << "File neg is empty.";
  // 随机乱序
  ...
  LOG(INFO) << "A total of " << pos_lines_.size() << " pos images.";
  LOG(INFO) << "A total of " << neg_lines_.size() << " neg images.";
    
  pos_lines_id_ = 0;
  neg_lines_id_ = 0;
  if (unified_data_param.rand_skip()) {
    unsigned int pos_skip = caffe_rng_rand() % unified_data_param.pos_rand_skip();
    unsigned int neg_skip = caffe_rng_rand() % unified_data_param.neg_rand_skip();
    LOG(INFO) << "Skipping pos first " << pos_skip << " instances.";
    LOG(INFO) << "Skipping neg first " << neg_skip << " instances.";
    CHECK_GT(pos_lines_.size(), pos_skip) << "Not enough pos points to skip";
    CHECK_GT(neg_lines_.size(), neg_skip) << "Not enough neg points to skip";
    pos_lines_id_ = pos_skip;
    neg_lines_id_ = neg_skip;
  }
  // data
...
template <typename Dtype>
void BBoxDataFsLayer<Dtype>::ShuffleLists_pos() {
  caffe::rng_t* prefetch_rng =
    static_cast<caffe::rng_t*>(prefetch_rng_->generator());
  shuffle(pos_lines_.begin(), pos_lines_.end(), prefetch_rng);
}

template <typename Dtype>
void BBoxDataFsLayer<Dtype>::ShuffleLists_neg() {
  caffe::rng_t* prefetch_rng =
    static_cast<caffe::rng_t*>(prefetch_rng_->generator());
  shuffle(neg_lines_.begin(), neg_lines_.end(), prefetch_rng);
}
    
template <typename Dtype>
void BBoxDataFsLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  LOG(INFO)<<"neg_lines_id_:"<<neg_lines_id_<<" "<<"pos_lines_id_:"<<pos_lines_id_;
  ...
  const int pos_lines_size = pos_lines_.size();
  const int neg_lines_size = neg_lines_.size();
  ...
  int num_pic_no_gt = 0; 
  //#######################################
  int num_neg = caffe_rng_rand() % max_neg_num_;
  LOG(INFO)<<"num_neg:"<<num_neg;
  int num_pos = batch_size - num_neg;
  LOG(INFO)<<"num_pos:"<<num_pos;
  for (int item_id = 0; item_id < num_pos; ++item_id) {
    CHECK_GT(pos_lines_size, pos_lines_id_);
    string xml_root = pos_lines_[pos_lines_id_].first;
    string xml_path = xml_root + '/' + pos_lines_[pos_lines_id_].second;
    ...
    pos_lines_id_++;
    if (pos_lines_id_ >= pos_lines_size) {
  ...
  // LOG(INFO)<<"num_neg"<<num_neg;
  for (int item_id = 0; item_id < num_neg; ++item_id) {
    CHECK_GT(neg_lines_size, neg_lines_id_);
    string xml_root = neg_lines_[neg_lines_id_].first;
    string xml_path = xml_root + '/' + neg_lines_[neg_lines_id_].second;
    // LOG(INFO)<<xml_root<<" "<<xml_path;
    // Read Anno
    AnnoData<Dtype> anno;
    if (unified_data_param.use_torsowithhead()) {
    ...
    images_all.push_back(image);
    neg_lines_id_++;
    if (neg_lines_id_ >= neg_lines_size) {
      DLOG(INFO) << "Restarting data prefetching from start.";
      neg_lines_id_ = 0;
      if (unified_data_param.shuffle()) {
        ShuffleLists_neg();
  ...
  // LOG(INFO) << images_all.size() << " " << batch_size;
...
template <typename Dtype>
void BBoxDataFsLayer<Dtype>::ReadTHAnnoDataFromXml(const int bindex, const string& xml_file, const string& root_dir,
    AnnoData<Dtype>* anno) {
  ptree pt;
  read_xml(xml_file, pt);
...
template <typename Dtype>
void BBoxDataFsLayer<Dtype>::ReadAnnoDataFromXml(const int bindex, const string& xml_file, const string& root_dir,
    AnnoData<Dtype>* anno) {
  ptree pt;
  read_xml(xml_file, pt);
  try {
...
INSTANTIATE_CLASS(BBoxDataFsLayer);
REGISTER_LAYER_CLASS(BBoxDataFs);
}  // namespace caffe
