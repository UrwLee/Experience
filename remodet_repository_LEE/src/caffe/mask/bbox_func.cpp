...
void DecodeBBoxes(const vector<vector<LabeledBBox<Dtype> > >& all_loc_preds,
   ...
   for (int i = 0; i < num; ++i) {
     vector<LabeledBBox<Dtype> >& decode_bboxes = (*all_decode_bboxes)[i];
...
void DecodeDenseBBoxes(const vector<vector<LabeledBBox<Dtype> > >& all_loc_preds,
   ...
   all_decode_bboxes->clear();
   all_decode_bboxes->resize(num);
   const int num_priors = all_loc_preds[0].size() / (num_classes - 1);
   for (int i = 0; i < num; ++i) {
...
void MatchAnchorsAndGTsRemoveLargeMargin(const vector<LabeledBBox<Dtype> > &gt_bboxes,
			...
			bool flag_only_w,float margin_ratio_h) {
  // LOG(INFO)<<"continue1111111";
  // 估计box的数量
  int num_pred = pred_bboxes.size();
  match_overlaps->clear();
  match_indices->clear();
...
//      match_indices_tmp[max_idx] = gt_indices[max_gt_idx];
  ...
  for(typename map<int,int>::iterator it = match_indices_tmp.begin(); it!=match_indices_tmp.end(); ++it){
    ...
    if(flag_only_w){
        ...
        if (margin_left<maring_w && margin_right<maring_w){
          (*match_indices)[ianchor] = igt;
        }
    }
    else{
        ...
        float maring_w = gt_w * margin_ratio;
        float maring_h = gt_h * margin_ratio_h;
        float margin_left = (float)(gt_b.x1_ - anchor_b.x1_);
        float margin_right = (float)(anchor_b.x2_ - gt_b.x2_);
...
void GetARMScores(const Dtype *conf_data, const int num, const int num_priors,
  ...
  neg_scores->clear();
  for (int i = 0; i < num; ++i) {
    vector<Dtype> pic_neg_scores;
    for (int p = 0; p < num_priors; ++p) {
      pic_neg_scores.push_back(conf_data[p * num_classes]);
    }
    neg_scores->push_back(pic_neg_scores);
    conf_data += num_priors * num_classes;
  }
}

template void GetARMScores(const float *conf_data, const int num, const int num_priors,
                           const int num_classes, const int loss_type,
                           vector<vector<float> > *neg_scores);
template void GetARMScores(const double *conf_data, const int num, const int num_priors,
                          const int num_classes, const int loss_type,
                          vector<vector<double> > *neg_scores);
...
void NmsOriSoft(const vector<LabeledBBox<Dtype> > &bboxes,
  ...
  while (score_index_vec.size() != 0) {
    ...
    score_index_vec.erase(score_index_vec.begin());
    typename vector<pair<Dtype, int> >::iterator it = score_index_vec.begin();
    while (it != score_index_vec.end()) {
      const int notkept_idx = it->second;
      Dtype iou = bboxes[idx].bbox.compute_iou(bboxes[notkept_idx].bbox);
      if ( iou >= nms_threshold) {
        it->first = it->first * (1.f - iou * iou);
        if (it->first < conf_threshold)
        {
          score_index_vec.erase(it);
          continue;
        }
      }
      ++it;
...
void NmsOriSoftThree(const vector<LabeledBBox<Dtype> > &bboxes,
  ...
  while (score_index_vec.size() != 0) {
    ...
    while (it != score_index_vec.end()) {
      ...
      Dtype iou = bboxes[idx].bbox.compute_iou(bboxes[notkept_idx].bbox);
      if ( iou >= nms_threshold) {
        it->first = it->first * (1.f - iou * iou * iou);
        if (it->first < conf_threshold)
        {
          score_index_vec.erase(it);
          continue;
...
void NmsOriSoftweight04WithVoting(vector<LabeledBBox<Dtype> > &bboxes,
  ...
  while (score_index_vec.size() != 0) {
    ...
    while (it != score_index_vec.end()) {
      ...
      Dtype iou = bboxes[idx].bbox.compute_iou(bboxes[notkept_idx].bbox);
      if (iou >= voting_thre) {
        box_voting[idx].push_back((bboxes)[notkept_idx].bbox);
        box_voting_score[idx].push_back(scores[notkept_idx]);
      }
...
template void NmsOriSoftweight04WithVoting(vector<LabeledBBox<float> > &bboxes,
    const vector<float> &scores,
    const float conf_threshold,
    const float nms_threshold,
    const int top_k,
    ...
    vector<int> *indices);
template void NmsOriSoftweight04WithVoting(vector<LabeledBBox<double> > &bboxes,
    const vector<double> &scores,
    const double conf_threshold,
    const double nms_threshold,
    const int top_k,
...
void NmsOri(const vector<LabeledBBox<Dtype> > &bboxes,
  ...
  while (score_index_vec.size() != 0) {
    ...
    while (it != score_index_vec.end()) {
      const int notkept_idx = it->second;
      if (bboxes[idx].bbox.compute_iou(bboxes[notkept_idx].bbox) >= nms_threshold) {
        score_index_vec.erase(it);
        continue;
      }
      ++it;
