...
void BBoxDataMultiResizeBatchTransformer<Dtype>::Transform(AnnoData<Dtype>& anno, cv::Mat* image,
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::Transform(AnnoData<Dtype>& anno, cv::Mat* image, bool* doflip) {
  randomExpand(anno, image);
...
//###########################################################
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::randomExpand(AnnoData<Dtype>& anno, cv::Mat* image) {
    // 读入图片
...
// randomly crop/scale
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::randomCrop(AnnoData<Dtype>& anno, cv::Mat* image, BoundingBox<Dtype>* crop_bbox) {
  // image
  //*image = cv::imread(anno.img_path.c_str());
  if (!image->data) {
    LOG(FATAL) << "Error when open image file: " << anno.img_path;
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::randomCrop(AnnoData<Dtype>& anno, cv::Mat* image) {
  // image
  //*image = cv::imread(anno.img_path.c_str());
  if (!image->data) {
    LOG(FATAL) << "Error when open image file: " << anno.img_path;
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::getCropBBox(AnnoData<Dtype>& anno, BoundingBox<Dtype>* crop_bbox) {
    ...
    h_max = 1.0;
    w_max = 1.0;
    int height = param_.resized_height_repeat(fetch_ratio_id_);
    int width = param_.resized_width_repeat(fetch_ratio_id_);
    if(width*9 == height*16){//16:9
      ...
      LOG(FATAL) << "Only support 16:9 or 9:16 of resize_width vs resize_heigth";
    }
        // get sampler
    if (phase_ == TRAIN) {
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::TransCrop(AnnoData<Dtype>& anno,
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::randomDistortion(cv::Mat* image, AnnoData<Dtype>& anno) {
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::randomFlip(AnnoData<Dtype>& anno, cv::Mat& image,
                            bool* doflip) {
  //生成０－１随机数
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::Normalize(AnnoData<Dtype>& anno) {
 // bboxÒÔ¼°kpsÈ«²¿¹éÒ»»¯
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::copy_label(AnnoData<Dtype>& anno, vector<BBoxData<Dtype> >* boxes) {
  boxes->clear();
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::fixedResize(AnnoData<Dtype>& anno, cv::Mat& image) {
//   // modify header
...
template<typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::visualize(cv::Mat& image, vector<BBoxData<Dtype> >& boxes) {
...
// show function
template<typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::visualize(AnnoData<Dtype>& anno, cv::Mat& image) {
  cv::Mat img_vis = image.clone();
  static int counter = 0;
  static const int color_maps[18] = {255,0,0,0,255,0,0,0,255,255,0,128,0,128,128,128,128,255};
  for (int i = 0; i < anno.instances.size(); ++i) {
...
template <typename Dtype>
void BBoxDataMultiResizeBatchTransformer<Dtype>::InitRand() {
...
template <typename Dtype>
int BBoxDataMultiResizeBatchTransformer<Dtype>::Rand(int n) {
  CHECK(rng_);
  CHECK_GT(n, 0);
  caffe::rng_t* rng =
...
INSTANTIATE_CLASS(BBoxDataMultiResizeBatchTransformer);
