...
void BBoxLossWTIOUCKCOVERLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  using_GHMC_loss_ = bbox_loss_param.using_gmhc_loss();
  using_twoclassbalance_loss_ = bbox_loss_param.using_twoclassbalance_loss();
  m_ = bbox_loss_param.mbins();
  gama_ = bbox_loss_param.gama();
  alpha_ = bbox_loss_param.alpha();
  weight = bbox_loss_param.weight();
  weight_type = bbox_loss_param.weight_type();
  use_group = bbox_loss_param.use_group();
  diff_thred = bbox_loss_param.diff_thred();
  power =  bbox_loss_param.power();
  k1 = bbox_loss_param.k1();
  k1 = bbox_loss_param.k2();
  b1 = bbox_loss_param.b1();
  b2 = bbox_loss_param.b2();
  only_pos_ = bbox_loss_param.only_pos();
  ...
  weight_mode_ = bbox_loss_param.weight_mode();
  ...
  if (conf_loss_type_ == MultiBoxLossParameter_ConfLossType_SOFTMAX) {
    //LOG(INFO)<<using_focus_loss_<<" "<<using_GHMC_loss_<<"--------------------";
    if (!using_focus_loss_ && !using_GHMC_loss_) {
      LayerParameter layer_param;
    ...
    } else if (using_focus_loss_ && !using_GHMC_loss_) {
      LayerParameter layer_param;
      ...
      LayerParameter layer_param;
      layer_param.set_name(this->layer_param_.name() + "_softmax_ghmc_conf");
      layer_param.set_type("GhmcLoss");
      layer_param.add_loss_weight(conf_weight_);
      layer_param.mutable_loss_param()->set_normalization(
        LossParameter_NormalizationMode_NONE);
      SoftmaxParameter* softmax_param = layer_param.mutable_softmax_param();
      softmax_param->set_axis(1);
      layer_param.mutable_ghmc_loss_param()->set_m(m_);
      layer_param.mutable_ghmc_loss_param()->set_alpha(alpha_);
      //layer_param.mutable_ghmc_loss_param()->set_alpha(weight);
      //layer_param.mutable_ghmc_loss_param()->set_alpha(weight_type); 
      vector<int> conf_shape(1, 1);
      conf_gt_.Reshape(conf_shape);
      conf_shape.push_back(num_classes_);
      conf_pred_.Reshape(conf_shape);
    ...
    if (!using_focus_loss_ &&
        !using_twoclassbalance_loss_ &&
        using_GHMC_loss_) {
      //LOG(INFO)<<"-------------------------";
      LayerParameter layer_param;
      layer_param.set_name(this->layer_param_.name() + "_logistic_ghmc_conf");
      layer_param.set_type("GHMCSigmoidCrossEntropyLoss");
      layer_param.add_loss_weight(conf_weight_);
      layer_param.mutable_ghmc_loss_param()->set_m(m_);
      layer_param.mutable_ghmc_loss_param()->set_alpha(alpha_);
      layer_param.mutable_ghmc_loss_param()->set_weight(weight);
      layer_param.mutable_ghmc_loss_param()->set_weight_type(weight_type);
      layer_param.mutable_ghmc_loss_param()->set_use_group(use_group);
      layer_param.mutable_ghmc_loss_param()->set_k1(k1);
      layer_param.mutable_ghmc_loss_param()->set_k2(k2);
      layer_param.mutable_ghmc_loss_param()->set_b1(b1);
      layer_param.mutable_ghmc_loss_param()->set_b2(b2);
      layer_param.mutable_ghmc_loss_param()->set_diff_thred(diff_thred);
      layer_param.mutable_ghmc_loss_param()->set_power(power);
      vector<int> conf_shape(1, 1);
      conf_shape.push_back(num_classes_);
      conf_gt_.Reshape(conf_shape);
      conf_pred_.Reshape(conf_shape);
      conf_loss_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
      conf_loss_layer_->SetUp(conf_bottom_vec_, conf_top_vec_);
    } else if (using_focus_loss_ &&
               !using_twoclassbalance_loss_ &&
               !using_GHMC_loss_) {
      LayerParameter layer_param;
      layer_param.set_name(this->layer_param_.name() + "_logistic_fl_conf");
      layer_param.set_type("FocalSigmoidCrossEntropyLoss");
      layer_param.add_loss_weight(conf_weight_);
      layer_param.mutable_focus_loss_param()->set_gama(gama_);
      layer_param.mutable_focus_loss_param()->set_alpha(alpha_);
      vector<int> conf_shape(1, 1);
      ...
      conf_loss_layer_->SetUp(conf_bottom_vec_, conf_top_vec_);
               ...
               !using_GHMC_loss_) {
      LayerParameter layer_param;
      layer_param.set_name(this->layer_param_.name() + "_logistic_bl_conf");
      layer_param.set_type("TwoClassBalancedSigmoidCrossEntropyLoss");
      layer_param.add_loss_weight(conf_weight_);
      layer_param.mutable_two_class_balanced_sigmoid_cross_entropy_loss_param()->set_only_pos(only_pos_);
      layer_param.mutable_two_class_balanced_sigmoid_cross_entropy_loss_param()->set_alpha(alpha_);
  ...
  conf_norm_pos_ = bbox_loss_param.conf_norm_pos();
  neg_max_value_ = bbox_loss_param.neg_max_value();
}
...
//  LOG(INFO)<<cnt<< " images for batchsizie "<<num_<<" In bbox_loss_layer";
  ...
  for (int i = 0; i < prior_bboxes.size(); i++) {
    ...
    weights_of_anchor_.clear();
  for (int i = 0; i < 13824; i++)
  {
    weights_of_anchor_.push_back(1);
  }
  for (int i = 0; i < 5184; i++)
  {
    weights_of_anchor_.push_back(2);
  }
  for (int i = 0; i < 1296; i++)
  {
    weights_of_anchor_.push_back(3);
  }
  /**************************************************************************#
  获取所有的Box坐标估计
  #***************************************************************************/
  ...
  for (int i = 0; i < num_; ++i) {
    ...
    bool flag_only_w = bbox_loss_param.only_w();
    float margin_ratio_h = (float)bbox_loss_param.margin_ratio_h();
    // 获得该样本的gt-box列表
    ...
    if (bbox_loss_param.matchtype_anchorgt() == BBoxLossParameter_MatchTypeAnchorGT_WEIGHTIOU) {
      ...
      MatchAnchorsAndGTsRemoveLargeMargin(gt_bboxes, prior_bboxes, match_type, overlap_threshold_, neg_overlap_,
                                          &match_overlaps, &match_indices, &unmatch_indices, flags_of_anchor_, flag_noperson_, flag_withotherpositive, margin_ratio, flag_only_w, margin_ratio_h);
...
//LOG(INFO)<<"All "<<num_neg_<<" negative instances;"<<num_pos_<<" positive instances bbox_loss_layer.";
  ...
  if (flag_noperson_) {
    ...
    //#######################################
    // LOG(INFO)<<"Num neg one minibatch:"<<num_neg_all;
    // LOG(INFO)<<"Num pos one minibatch:"<<num_pos_;
    //#######################################
    ...
    if (do_neg_mining_) {
      ...
      if (neg_max_value_!=0){
        num_neg = std::min(num_neg, neg_max_value_);
      }
    } else {
      CHECK_EQ(num_neg, scores_indices.size());
    }
    std::sort(scores_indices.begin(), scores_indices.end(), PairDescend<pair<int, int>, Dtype>);
    for (int i = 0; i < num_; ++i) {
      vector<int> neg_indices;
      ...
      for (int i = 0; i < num_; ++i) {
        if (all_pos_indices_[i].size() > 0) {
          ...
          for (map<int, int>::const_iterator it = match_indices.begin();
            ...
            if (weight_mode_==1)
            {
              if (weights_of_anchor_[prior_id]==3)
              {
                Dtype scale_weight = 1.5;
                caffe_scal(4, scale_weight, loc_pred_.mutable_cpu_diff()+count*4);
              }
            }
    ...
    if (num_conf_ >= 1) {
      ...
      for (int i = 0; i < num_; ++i) {
        ...
        for (map<int, int>::const_iterator it =
          ...
          CHECK_LT(prior_id, num_priors_);
          if (weight_mode_==2){
            if (weights_of_anchor_[prior_id]==1 || weights_of_anchor_[prior_id]==2){
              Dtype scale_weight = 3;
              caffe_scal(num_classes_, scale_weight, conf_pred_.mutable_cpu_diff()+ count * num_classes_);
            }
          }
          if (weight_mode_==3){
            if (weights_of_anchor_[prior_id]==3){
              Dtype scale_weight = 0.5;
              caffe_scal(num_classes_, scale_weight, conf_pred_.mutable_cpu_diff()+ count * num_classes_);
            }
          }
          if (weight_mode_==4){
