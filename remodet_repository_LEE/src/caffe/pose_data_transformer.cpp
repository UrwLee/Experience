...
bool PoseDataTransformer<Dtype>::ReadMetaDataFromXml(const string& xml_file, const string& root_dir, MetaData& meta) {
  ...
  if (meta.dataset.find("COCO") != string::npos) {
    np_ = 17;
  } else if (meta.dataset.find("Google") != string::npos) {
    np_ = 17;
  } else if (meta.dataset.find("AIC") != string::npos) {
    np_ = 17;
...
void PoseDataTransformer<Dtype>::Transform_nv(cv::Mat& img,
  ...
  for (int g_y = 0; g_y < grid_y; g_y++) {
    for (int g_x = 0; g_x < grid_x; g_x++) {
      ...
      for (int i = 0; i < 18; i++) {
        if (np_ == 17) {
          if (param_.mode_data()==0){
            // COCO (眼睛鼻子也反传loss) / Google / AIC
            transformed_heat_mask[i * channelOffset + g_y * grid_x + g_x] = mask;
          }else{
            if ((meta.dataset.find("COCO") != string::npos) && ((i==0) || (i==14) || (i==15))){
              // COCO 眼睛和鼻子，不反传loss
              transformed_heat_mask[i * channelOffset + g_y * grid_x + g_x] = 0;
            }else{
              // COCO (不含眼鼻) / Google / AIC
              transformed_heat_mask[i * channelOffset + g_y * grid_x + g_x] = mask;
            }
          }
        } else if (np_ == 16) {
      ...
      for (int i = 0; i < 17 * 2; ++i) {
        if (np_ == 17) {
          if (param_.mode_data()==0){
            ...
            if ((meta.dataset.find("COCO") != string::npos) && (i > 23)){
              // COCO 眼睛和鼻子，不反传loss
...
template <typename Dtype>
bool PoseDataTransformer<Dtype>::bg_crop(MetaData& meta) {
  // 判断是否为背景, 即所有kp不可见.
  for(int i = 0; i < meta.joint_self.isVisible.size(); i++){
    if (meta.joint_self.isVisible[i] != 2){
      return false;
    }
  }

  const BGCropParameter& param = param_.bg_crop_param();
  float dice = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
  if (dice <= param.prob()){

      float dice_h, dice_w, dice_x1, dice_y1;
      caffe_rng_uniform(1, param.w_min(), param.w_max(), &dice_w);
      caffe_rng_uniform(1, param.h_min(), param.h_max(), &dice_h);
      caffe_rng_uniform(1, 0.f, (1.f-dice_w), &dice_x1);
      caffe_rng_uniform(1, 0.f, (1.f-dice_h), &dice_y1);

      float xmin   = dice_x1 * meta.img_size.width;
      float ymin   = dice_y1 * meta.img_size.height;
      float width  = dice_w  * meta.img_size.width;
      float height = dice_h  * meta.img_size.height;

      meta.bbox.x1_ = xmin;
      meta.bbox.y1_ = ymin;
      meta.bbox.x2_ = meta.bbox.x1_ + width;
      meta.bbox.y2_ = meta.bbox.y1_ + height;

      meta.objpos.x = xmin+width/2.0;
      meta.objpos.y = ymin+height/2.0;
      meta.scale_self = height/368.0;
      meta.area =  width*height;
  }
