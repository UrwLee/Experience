...
// // Get convolutionNHWC layer according to engine.
// template <typename Dtype>
// shared_ptr<Layer<Dtype> > GetConvolutionNHWCLayer(
//     const LayerParameter& param) {
//   ConvolutionParameter conv_param = param.convolution_param();
//   ConvolutionParameter_Engine engine = conv_param.engine();
//   // Note: we only use caffe
//   engine = ConvolutionParameter_Engine_CAFFE;
//   if (engine == ConvolutionParameter_Engine_CAFFE) {
//     return shared_ptr<Layer<Dtype> >(new ConvolutionNHWCLayer<Dtype>(param));
//   } else {
//     LOG(FATAL) << "Layer " << param.name() << " has unknown engine.";
//   }
// }
//
// REGISTER_LAYER_CREATOR(ConvolutionNHWC, GetConvolutionNHWCLayer);
