...
#include "caffe/pose/new_pose_det_layer.hpp"
#include "caffe/util/bbox_util.hpp"

namespace caffe {

template <typename Dtype>
void NewPoseDetLayer<Dtype>::LayerSetUp(
  ...
  history_.resize(7+num_parts_*3);
...
template <typename Dtype>
void NewPoseDetLayer<Dtype>::Reshape(const vector<Blob<Dtype> *> &bottom,
...
template <typename Dtype>
void NewPoseDetLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {
  const Dtype UNFOUND_THRE = 0.05;
  const Dtype MAX_SEARCH_DIS = 0.001;
  const Dtype* pose_data = bottom[0]->cpu_data();
  const Dtype* det_data = bottom[1]->cpu_data();
  const Dtype* peak_data = bottom[2]->cpu_data();
  ...
  if (num_pose_person > 1) num_pose_person = 1;
  ...
  if (num_det_person > 1) num_det_person = 1;
    ...
    if (matched_idx >= 0) {
      ...
      for (int k = 0; k < num_parts_; ++k) {
        ...
        if (vk > 0.05) {
          ...
          // 历史值不可见，丢弃
          if (history_[4+3*k+2] < 0.05) continue;
          // 历史位置
          float hx = history_[4+3*k];
          float hy = history_[4+3*k+1];
          /**
           * 搜索最近邻的候选点, 距离不能超过设定限制
           */
          ...
          for (int nk = 1; nk <= max_peaks_k; ++nk) {
            ...
            // 区域外和低置信度，直接丢弃
            if (vp < max_val) continue;
            if ((xp < xmin_det) || (xp > xmax_det) || (yp < ymin_det) || (yp > ymax_det)) continue;
            // 计算距离
            float dis = (xp-hx)*(xp-hx) + (yp-hy)*(yp-hy);
            // 符合限制
            if (dis < min_dis) {
              min_dis = dis;
              max_p = nk;
          ...
          if (max_p >= 1) {
            ...
            // 更新历史值
            history_[4+3*k] = x_k;
            history_[4+3*k+1] = y_k;
            history_[4+3*k+2] = v_k;
          }
        }
      }
      ...
      for (int k = 0; k < num_parts_; ++k) {
        ...
        if (vk > 0.05) {
         ...
         for (int nk = 1; nk <= max_peaks_k; ++nk) {
           ...
           // 区域外和低置信度，直接丢弃
           if (vp < max_val) continue;
           if ((xp < xmin_pose) || (xp > xmax_pose) || (yp < ymin_pose) || (yp > ymax_pose)) continue;
           // 计算距离
           float dis = (xp-hx)*(xp-hx) + (yp-hy)*(yp-hy);
           // 符合限制
           if (dis < min_dis) {
  ...
  for (int i = 0; i < det_bboxes.size(); ++i) {
    ...
    out_vec[2] = xmax_det;
    out_vec[3] = ymax_det;
    // 存在检测到的pose对象，则跳过，不处理
    if (pose_bboxes.size() == 0) {
      int n_points = 0;
      for (int k = 0; k < num_parts_; ++k) {
        // 历史值不可见，丢弃
        if (history_[4+3*k+2] < 0.05) continue;
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(NewPoseDetLayer, Forward);
#endif

INSTANTIATE_CLASS(NewPoseDetLayer);
REGISTER_LAYER_CLASS(NewPoseDet);
