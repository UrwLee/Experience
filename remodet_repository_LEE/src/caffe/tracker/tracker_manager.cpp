...
TrackerManager<Dtype>::TrackerManager(const std::vector<Video<Dtype> >& videos,
                               RegressorBase<Dtype>* regressor, TrackerBase<Dtype>* tracker)
  : videos_(videos),regressor_(regressor), tracker_(tracker) {
}

template <typename Dtype>
void TrackerManager<Dtype>::TrackAll() {
  TrackAll(0, 1);
}

template <typename Dtype>
void TrackerManager<Dtype>::TrackAll(const int start_video_num, const int pause_val) {
  // 遍历迭代每个视频序列
  for (int video_num = start_video_num; video_num < videos_.size(); ++video_num) {
    ...
    // 初始化Tracker
    tracker_->Init(image_curr, bbox_gt);
    // 跟踪下面的所有帧
    for (int frame_num = first_frame + 1; frame_num < video.all_frames_.size(); ++frame_num) {
      ...
      // 使用当前图像进行估计
      tracker_->Tracking(image_curr, regressor_, &bbox_estimate_uncentered);
...
INSTANTIATE_CLASS(TrackerManager);
