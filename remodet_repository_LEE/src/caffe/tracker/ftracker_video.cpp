#include "caffe/tracker/ftracker_video.hpp"
...
static std::string windowName = "REMO FTracker";

void FonMouse(int event, int x, int y, int, void* param) {
...
FVideoTracker<Dtype>::FVideoTracker(const VideoTrackerParameter& param,
                                   FRegressorBase<Dtype>* fregressor,
                                   FTrackerBase<Dtype>* ftracker)
  : param_(param),fregressor_(fregressor), ftracker_(ftracker) {
  // 视频类型：VIDEO & WEBCAM
  ...
  cv::setMouseCallback(windowName, FonMouse, (void*)&cv_img);
  ...
  box.y2_ = select_roi.y + select_roi.height;
  // 复位Tracker
  ftracker_->Init(cv_img, box);
  LOG(INFO) << "The ftracker has been initialized.";
  // 初始化保存
  if (param_.save_videos()) {
    ...
    if (is_type_video_) {
      ...
      LOG(INFO) << "Saving FVideoTracker Results for: " << video_name;
      const string& video_out_folder = param_.output_folder() + "/FVTResults_Video";
      ...
      const string& video_out_folder = param_.output_folder() + "/FVTResults_Webcam";
      boost::filesystem::create_directories(video_out_folder);
...
// Tracking
template <typename Dtype>
void FVideoTracker<Dtype>::Tracking() {
  while(1) {
    cv::Mat cv_img;
    if (is_type_video_) {
    ...
    ftracker_->Tracking(cv_img, fregressor_, &bbox_estimated);
...
template <typename Dtype>
void FVideoTracker<Dtype>::ProcessTrackOutput(
...
template <typename Dtype>
void FVideoTracker<Dtype>::PostProcessAll() {
  if (is_type_video_) {
    LOG(INFO) << "Finished tracking the video.";
  }
}

INSTANTIATE_CLASS(FVideoTracker);
