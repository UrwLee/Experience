...
TrackerViewer<Dtype>::TrackerViewer(const std::vector<Video<Dtype> >& videos,
                                    RegressorBase<Dtype>* regressor,
                                    TrackerBase<Dtype>* tracker,
                                    const bool save_videos,
                                    const bool save_outputs,
                                    const std::string& output_folder)
 : TrackerManager<Dtype>(videos,regressor,tracker), output_folder_(output_folder), num_frames_(0), save_videos_(save_videos), save_outputs_(save_outputs) {
}

template <typename Dtype>
void TrackerViewer<Dtype>::VideoInit(const Video<Dtype>& video, const int video_num) {
...
template <typename Dtype>
void TrackerViewer<Dtype>::ProcessTrackOutput(
...
template <typename Dtype>
void TrackerViewer<Dtype>::PostProcessVideo() {
  if (save_outputs_) {
...
template <typename Dtype>
void TrackerViewer<Dtype>::PostProcessAll() {
  LOG(INFO) << "Finished tracking " << this->videos_.size() << " videos.";
}

INSTANTIATE_CLASS(TrackerViewer);
