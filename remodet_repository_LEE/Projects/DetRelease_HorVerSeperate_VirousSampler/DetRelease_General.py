...
only_w = True
###MergeSingePerson
single_person_size=10
merge_single_person_prob=0.0
###body
...
if train_net_id == 0:
    ...
    lr_inter_loss = 1.0
    lr_pose = 0.1
    min_scale = [1.75,1.45,1.1,0.75]#default [0.75]*4 
    max_scale = [2.0,1.75,1.45,1.1]#default [2.0]*4
...
if train_net_id == 0:#train Body and Part
    ...
    Models = "%s_BDPDBBoxDistP%s%s%s_BD%sLWLoc%sConf%sPIOU%sNIOU%sR%d" \
             %(model_pre,str(dist_prob),noperson_str,data_str_body,bdloss_str,str(bboxloss_loc_weight_body),str(bboxloss_conf_weight_body),
               str(bboxloss_overlap_threshold_body),str(bboxloss_neg_overlap_body),bboxloss_neg_pos_ratio_body)

    if merge_single_person_prob!=0.0:
