...
def addconv6(net, from_layer="conv5", use_bn=True, conv6_output=[],conv6_kernal_size=[],start_pool=True,leaky=False,lr_mult=1,
			 decay_mult=1,pre_name="conv6",post_name="",n_group=1):
	if start_pool:
		out_layer = 'pool5'
	...
	for i in range(len(conv6_output)):
		...
		ConvBNUnitLayer(net, from_layer, out_layer, use_bn=use_bn, use_relu=True, \
				num_output=num_output, kernel_size=kernel_size, pad=(kernel_size-1)/2, stride=1, use_scale=True,
						leaky=leaky,lr_mult=lr_mult, decay_mult=decay_mult,n_group=n_group,truncvalue=trunc_v)
