...
if USERNAME == "zhangming":
    ...
    val_list = ['/home/%s/Datasets/AIC_Data/Layout/val_PersonWithFaceHeadHand_V0_V0_cont1_V0_cont2_V0_cont3.txt' % USERNAME,
                '/home/%s/Datasets/AIC_Data/Layout/val_largebox.txt' % USERNAME]
    root_dir_val = ['/home/%s/Datasets/AIC_Data' % USERNAME,'/home/%s/Datasets/AIC_Data' % USERNAME]
else:
...
useHisiData = True
hisiDataMapPath = '/home/zhangming/Datasets/AIC_Data/Layout/hisiDataMap.txt'
...
def get_unifiedTransParam(train=True):
    if train:
        ...
        if max_expand_ratio>1.0:
            param_dic["expand_param"] = {"prob":0.5,"max_expand_ratio":max_expand_ratio}
        return param_dic
    else:
        return {
                'sample_sixteennine': flag_sample_sixteennine,
...
def get_unified_data_param(train=True,batchsize=1,id_val=0):
    if train:
        ...
        if isinstance(val_list, list):
            return {
                'xml_list': val_list[id_val],
                'xml_root': root_dir_val[id_val],
                'shuffle': True,
                'rand_skip': 1,
                'batch_size': 1,
                'mean_value': [104, 117, 123],
...
def get_DAPDataLayer(net, train=True, batchsize=1,id_val=0):
    if train:
        ...
        unifiedDataParam = get_unified_data_param(train=train, batchsize=batchsize, id_val=id_val)
    if flag_hisi_map:
        unifiedDataParam["hisi_data_maps"] = "/home/zhangming/Datasets/AIC_Data/Layout/hisiDataMap.txt"
    if train:
        kwargs = {
