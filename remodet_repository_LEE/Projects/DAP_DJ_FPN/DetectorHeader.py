...
def UnitLayerDenseDetectorHeader(net, data_layer="data", num_classes=2, feature_layer="conv5", \
    ...
    if not inter_layer_channels==0:
        if len(inter_layer_channels) > 0:
            ...
            for inter_channel_kernel in inter_layer_channels:
                ...
                ConvBNUnitLayer(net, from_layer, inter_name, use_bn=use_batchnorm, use_relu=True, \
                    num_output=inter_channel, kernel_size=inter_kernel, pad=inter_pad, stride=1,use_scale=True, leaky=False,
                                lr_mult=lr_mult, decay_mult=decay_mult,truncvalue=trunc_v,flag_withparamname=flag_withparamname)
                from_layer = inter_name
                start_inter_id = start_inter_id + 1
    # PriorBoxes
    num_priors_per_location = len(pro_widths)
    ...
    if name in truncvalues.keys():
        ...
        trunc_v = -1
    ConvBNUnitLayer(net, from_layer, name, use_bn=False, use_relu=False,num_output=num_loc_output, kernel_size=3, pad=1,
                    stride=1,lr_mult=lr_mult, decay_mult=decay_mult,truncvalue = trunc_v,flag_withparamname=flag_withparamname)
    permute_name = "{}_perm".format(name)
    net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
    if flat:
        flatten_name = "{}_flat".format(name)
    ...
    if use_focus_loss:
        ...
        ConvBNUnitLayer(net, from_layer, name, use_bn=False, use_relu=False, \
            num_output=num_conf_output, kernel_size=3, pad=1, stride=1,lr_mult=lr_mult, decay_mult=decay_mult,truncvalue = trunc_v,flag_withparamname=flag_withparamname)
    permute_name = "{}_perm".format(name)
    net[permute_name] = L.Permute(net[name], order=[0, 2, 3, 1])
    if flat:
        flatten_name = "{}_flat".format(name)
      ...
      for j in range(len(boxsizes_per_layer)):
        ...
        for each_aspect_ratio in aspect_ratio:
            if isinstance(boxsize,list):
                w = boxsize[0]*math.sqrt(each_aspect_ratio)
                h = boxsize[1] / math.sqrt(each_aspect_ratio)
            else:
