...
void BBoxDataHandKeypointTransformer<Dtype>::Transform(AnnoData<Dtype>& anno, cv::Mat* image,
  ...
  get_mask(anno, *image,transformed_heatmap);
...
template<typename Dtype>
void BBoxDataHandKeypointTransformer<Dtype>::get_mask(AnnoData<Dtype>& anno, cv::Mat& image,Dtype* transformed_heatmap){
  int rezX = image.cols;
  int rezY = image.rows;
  int stride=param_.stride();
...
// LOG(INFO)<<1111;
  ...
  generateLabelMap(transformed_heatmap, image, anno);
}
template<typename Dtype>
void BBoxDataHandKeypointTransformer<Dtype>::generateLabelMap(Dtype* transformed_heatmap,
