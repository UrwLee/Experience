#ifndef CAFFE_MASK_BBOX_DATA_MULTI_RESIZE_LAYER_HPP_
#define CAFFE_MASK_BBOX_DATA_MULTI_RESIZE_LAYER_HPP_
...
class BBoxDataMultiResizeLayer : public BasePrefetchingDataLayer<Dtype> {
 public:
  explicit BBoxDataMultiResizeLayer(const LayerParameter& param)
      : BasePrefetchingDataLayer<Dtype>(param), bbox_data_transform_param_(param.unified_data_transform_param()) {}
  virtual ~BBoxDataMultiResizeLayer();
  virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "BBoxDataMultiResize"; }
 ...
 protected:
  ...
  // 数据转换器
  shared_ptr<BBoxDataMultiResizeTransformer<Dtype> > bbox_data_transformer_;
