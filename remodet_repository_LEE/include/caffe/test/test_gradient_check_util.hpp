    ...
    for (int feat_id = 0; feat_id < current_blob->count(); ++feat_id) {
      ...
      if (kink_ - kink_range_ > fabs(feature)
        ...
        Dtype scale = std::max<Dtype>(
            ...
            Dtype(1.));
        // Dtype err = computed_gradient - estimated_gradient;
        // if (fabs(err) >= threshold_ * scale) {
        //   LOG(WARNING) << "[ FAIL] debug: (top_id, top_data_id, blob_id, feat_id)="
        //                << top_id << "," << top_data_id << "," << blob_id << "," << feat_id
        //                << "; feat = " << feature
        //                << "; objective+ = " << positive_objective
        //                << "; objective- = " << negative_objective;
        // } else {
        //   LOG(INFO) << "[   OK] (top_id, top_data_id, blob_id, feat_id)="
        //             << top_id << "," << top_data_id << "," << blob_id << ","
        //             << feat_id;
        // }
        // DLOG(INFO) << "[" << top_id << "," << top_data_id << "," << blob_id << "," << feat_id << "] "
        //           << ">> computed_gradient: " << computed_gradient << ", estimated_gradient: " << estimated_gradient;
        EXPECT_NEAR(computed_gradient, estimated_gradient, threshold_ * scale)
          << "[FAIL] debug: (top_id, top_data_id, blob_id, feat_id)="
          << top_id << "," << top_data_id << "," << blob_id << "," << feat_id
          << "; feat = " << feature
          << "; objective+ = " << positive_objective
          << "; objective- = " << negative_objective;
