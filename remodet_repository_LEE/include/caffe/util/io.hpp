...
inline void makeHisiDataMaps(const std::string& path, std::map<int, int>* maps) {
  std::ifstream tempfile(path.c_str());
  CHECK(tempfile.good()) << "Failed to open file " << path;
  int key, value;
  while (tempfile >> key >> value) {
    (*maps)[key] = value;
  }
}
...
cv::Mat ReadImage(const string& filename, const int height, const int width,
